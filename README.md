...
| `pi-delegate` | Explicitly delegate and monitor bounded research, analysis, review, or implementation work in Pi with the fixed GLM-5.2 model while Codex verifies the result. |
...
- `implement-feature` has a tracker-only discovery path for availability/listing requests; it reports candidate Specs and explicitly leaves execution eligibility unverified without loading startup contracts or creating state, claims, tasks, worktrees, or tracker changes. Explicit execution requires local `python3` for its shipped `scripts/run-state` SQLite coordinator, read-only `scripts/verify-ready` terminal verifier, and `$autoreview` for every delivery; GitStack is required only for GitHub transport. AutoReview owns the review profile and adds one native Codex review only for high-risk changes. The controller task may be bound to a multi-folder local Codex project, but that project's primary-folder Git defaults and attached folders are control-plane and read-only coordination context only. When task readback omits `projectId`, a unique exact same-host local Git path and matching Git common directory may resolve a controller-only compatibility identity; the ChatGPT App does not expose folder cardinality, so that fallback does not claim a single-folder project and never satisfies worker mapping. Before state, Implement Feature maps every affected repository bijectively to its own separate repo-specific saved Git project whose reported primary folder and Git common directory match the repository; the multi-folder controller project never satisfies that worker mapping. Remote, broad, duplicate, ambiguous, or missing mappings stop unless the one startup interaction explicitly authorizes creation of the exact missing projects. That interaction explicitly distinguishes future conditional permission for a separate Plan Feature repair task from immediate task creation. Workers own implementation, validation, commits, review candidates, finding acceptance and fixes, tracker proof, and either GitHub PR or named local-branch closeout; canonical `review_owner=worker|root` is persisted through reconciled ChatGPT App task operations and owns AutoReview execution only. The root coordinates every worker allowed by path and dependency serialization without a fixed numeric cap and uses one permanently unversioned per-user database-schema-4 DB at `~/.cache/dotagents/skills/implement-feature/run-state.sqlite3`; CLI, runtime-contract, and named JSON protocol SemVer identities remain separate from that SQLite schema integer. Runs pin their exact runtime artifact, create each worker with its final canonical title and verify that title in the same operation, and use stable bootstrap and scope-revision IDs for replay-safe authority effects. On the first required out-of-envelope path, the root can retain the same worker, claim, task, worktree, and bootstrap while a separate Plan Feature task monotonically repairs GitHub-backed `allowed_paths`; root then recomputes complete-envelope overlap and delivers contract generation `N+1`. Local planning reports an explicit transport blocker, a second miss requires full replanning, and no per-file claims are created. The single-row `runtime_metadata` table and SQLite transactions fence no-migration hard cuts without an external lock file, while canonical Feature Spec and head-branch claims prevent duplicate work and different roots may use distinct worktrees in the same repository. Root and workers never author planning artifacts, switch the main worktree, or merge pull requests.
