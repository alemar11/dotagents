...
- `implement-feature` requires local `python3` for its shipped `scripts/run-state` SQLite coordinator, read-only `scripts/verify-ready` terminal verifier, and `$autoreview` for every delivery; GitStack is required only for GitHub transport. AutoReview owns the review profile and adds one native Codex review only for high-risk changes. Before state, Implement Feature verifies that every affected repository is a saved Git project capable of receiving a ChatGPT-created worktree; missing projects require explicit setup authority in the one startup interaction or the run stops. That interaction also covers visible Codex tasks and their assigned worktrees. Workers own implementation, validation, commits, review candidates, finding acceptance and fixes, tracker proof, and either GitHub PR or named local-branch closeout; canonical `review_owner=worker|root` is persisted through reconciled App operations and owns AutoReview execution only. The root coordinates at most three disjoint workers and uses one permanently unversioned per-user database-schema-2 DB at `~/.cache/dotagents/skills/implement-feature/run-state.sqlite3`; CLI, runtime-contract, and named JSON protocol SemVer identities remain separate from that SQLite schema integer. Runs pin their exact runtime artifact, generated operation IDs reconcile ChatGPT task changes, and one stable bootstrap ID gives replayable exactly-once authority effect. The single-row `runtime_metadata` table and SQLite transactions fence no-migration hard cuts without an external lock file, while canonical Feature Spec and head-branch claims prevent duplicate work and different roots may use distinct worktrees in the same repository. It never plans, repairs planning artifacts, switches the main worktree, or merges pull requests.
