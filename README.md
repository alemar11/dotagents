...
| `se2:feature` | Traverse the graph-first Feature workflow, then converge linked Feature issues and vertical Task graphs in the invoking local project without creating a worktree; publish is the default and preview is explicit opt-in. |
...
| `se2:implement` | Execute authoritative GitHub Features through a mandatory GitHub-backed graph-first orchestrator, isolated workers, exact-HEAD review, Contract Repair, and verified standalone or stacked PR delivery. |
...
- `se2:implement` accepts only complete authoritative GitHub Feature bundles and returns a verified standalone or stacked PR topology. GitHub interaction is mandatory end to end; it has no local-only or preview execution mode. A Sol/medium orchestrator derives delivery dependencies separately from execution waves: serialization alone never creates a stack, while a true same-repository code dependency may start only from a green, exact-HEAD-verified parent PR. Workers own implementation semantics and validation; G owns Git transport, PR publication, hosted review operations, and pairwise stack linking; parent drift invalidates descendant evidence for bottom-to-top worker reconciliation. A minimal SQLite WAL ledger prevents concurrent orchestrators on the same Feature and stores durable checkpoints and idempotent side effects only; schema changes use explicit drop-and-recreate, never migrations.
