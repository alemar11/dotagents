...
- `se2:implement` accepts only complete authoritative GitHub Feature bundles with stable bracketed acceptance-criterion IDs and returns a verified standalone or stacked PR topology. GitHub interaction is mandatory end to end; it has no local-only or preview execution mode. A Sol/medium orchestrator derives delivery dependencies separately from execution waves: serialization alone never creates a stack, while a true same-repository code dependency may start only from a green, exact-HEAD-verified parent PR. Workers own implementation semantics, validation, and Task criterion evidence bound to the exact candidate HEAD; the orchestrator aggregates Feature criterion coverage without rewriting issue bodies. Idea, Feature, and Implement apply one shared portable-content gate immediately before each hosted write; G owns transport and readback rather than semantic cleanup. Parent drift invalidates descendant evidence for bottom-to-top worker reconciliation. A minimal SQLite WAL ledger prevents concurrent orchestrators on the same Feature and stores durable checkpoints and idempotent side effects only; schema changes use explicit drop-and-recreate, never migrations.
