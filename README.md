...
| `se2:feature` | Analyze one or more source issues into linked textual Feature Plans, use optional read-only analysts and a critic when delegation is available, present one consolidated question batch, and publish by default without creating a worktree or implementation execution graph. |
...
| `se2:implement` | Execute authoritative GitHub Feature Plans through a mandatory GitHub-backed orchestrator, derive execution units in the control plane, use isolated workers, and return verified standalone or stacked PR delivery. |
...
- `se2:implement` accepts only complete authoritative GitHub Feature Plans with stable Feature acceptance IDs and returns a verified standalone or stacked PR topology. GitHub interaction is mandatory end to end; it has no local-only or preview execution mode. A Sol/medium orchestrator interprets each textual plan, derives execution units and delivery dependencies separately from runtime waves, and creates isolated Feature Workers. Serialization alone never creates a stack, while a true same-repository code dependency may start only from a green, exact-HEAD-verified parent PR. Workers own implementation semantics, validation, and Feature-criterion evidence bound to the exact candidate HEAD; the orchestrator aggregates plan evidence without rewriting the plan. Product-level contradictions are surfaced to the user rather than causing an automatic Feature-planning relaunch. Final provider readiness is supplied by `g:github-delivery-status`, whose `ready` and `ready-with-manual-action` dispositions do not grant Implement merge, auto-merge, bypass, or queue authority. Idea, Feature, and Implement apply one shared portable-content gate immediately before each hosted write; G owns transport and readback rather than semantic cleanup. Parent drift invalidates descendant evidence for bottom-to-top worker reconciliation. A minimal SQLite WAL ledger prevents concurrent orchestrators on the same Feature Plan and stores durable checkpoints and idempotent side effects only; schema changes use explicit drop-and-recreate, never migrations.
...
- Multi-repository runs additionally validate the complete linked Feature Plan set and finish with one independently verified GitHub PR per repository plus one exact HEAD vector.
