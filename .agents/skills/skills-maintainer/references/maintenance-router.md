...
## Request Types
- `maintain`: inspect one or more existing skills, detect drift, and apply the right maintenance mode
...
## Decision Tree
1. If the user invokes `$skills-maintainer` generically with a bare imperative such as `run`, `run your tasks`, or `do a maintenance pass` and does not name a task, classify as `maintain` and use `run-maintenance.md`.
...
2. If the user asks to maintain, upgrade, modernize, tighten, or improve one or more named existing skills, classify as `maintain` and use `skill-upgrade.md`.
