...
- Auth/session errors (`gh auth status` fails, 401/403 auth):
  - Retry command:
    `gh auth login && scripts/core/preflight_gh.sh --host github.com`
...
- Repository mismatch errors (`--expect-repo` does not match current
  directory):
  - Retry command:
    `scripts/core/preflight_gh.sh --host github.com --expect-repo owner/repo`
    from the target repo root, or use `scripts/triage/issues_copy.sh` /
    `scripts/triage/issues_move.sh` with explicit repo arguments for
...
- PR edit scope errors (`gh pr edit` fails with
  ...
  - Retry command:
    `scripts/triage/prs_update.sh --pr <n> [--title ...] [--body ...] [--base ...] [--repo owner/repo]`
