...
- The repeated workflow already overlaps a current helper in this GitHub skill
  package or one of its internal domains.
...
- Document every promoted helper in the owning domain's `script-summary.md`
...
- Release creation now uses dedicated planning, notes-generation, and
  release-creation helpers with explicit notes strategy handling in the
  `releases` domain.
- `gh pr edit` may require `read:project`; `scripts/triage/prs_update.sh`
  should stay the fallback for title/body/base-only updates when that scope is
  missing.
