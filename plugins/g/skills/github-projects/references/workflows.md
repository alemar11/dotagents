...
resolving `<plugin-root>`, and completing the Projects checks in
[the GitHub CLI preflight](../../../references/gh-dependency-preflight.md). The
[invocation registry](../../../references/options.md) is the sole owner of
...
`<provider-query>` as provider-native query syntax, not a G option value. Pass
the complete query as one shell-quoted argument. Resolve an owner-qualified
