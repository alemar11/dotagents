...
def _print_write_text(payload: dict[str, object]) -> int:
    ...
    for item in payload.get("results") or []:
        if isinstance(item, dict):
            print(f"- {item.get('repo')}: {item.get('message') or item.get('status')}")
    return 0
...
def _run_list(args: argparse.Namespace) -> int:
    ...
    return _emit(payload) if args.json else _print_read_text(payload)
...
def _run_write(args: argparse.Namespace, add: bool) -> int:
    ...
    for repo in repos:
        ...
        try:
            ...
            result.update(
                ...
                wasStarred=already_starred,
            )
            if add and already_starred:
                result.update(status="noop", message="already starred")
            elif not add and not already_starred:
                result.update(status="noop", message="already unstarred")
            elif args.dry_run:
                result.update(
                    status="dry-run", message="would star" if add else "would unstar"
                )
            else:
                _mutate_star(repo_id, add=add)
                result.update(
                    status="changed", message="starred" if add else "unstarred"
                )
    ...
    (_emit if args.json else _print_write_text)(payload)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    actions.add_argument("--list-stars", action="store_true")
    actions.add_argument("--star", action="store_true")
    actions.add_argument("--unstar", action="store_true")
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return _run_write(args, add=bool(args.star))
    except GhError as exc:
