"""Closed typed operation protocol for managed G review work."""
...
RESULT_SCHEMA = "g-review-operation-result:v1"
...
def build_request(*, operation: str, controller_envelope: dict[str, Any], target: dict[str, Any], input_value: dict[str, Any]) -> dict[str, Any]:
    ...
    if descriptor != {"owner": "g", "name": operation, "contract_version": "1.0.0"}:
        raise OperationError("controller operation does not match the G request")
...
def validate_start_receipt(value: Any, request: dict[str, Any]) -> dict[str, Any]:
    ...
    if item["schema"] != START_RECEIPT_SCHEMA or item["owner"] != "g":
...
def validate_start_receipt_identity(
    ...
    """Validate a G receipt when only immutable operation identity remains."""
...
def validate_started_mutation(value: Any) -> dict[str, Any]:
    ...
    if request["operation"] not in {"request", "warning", "reply", "resolve"}:
        raise OperationError("started operation is not a G mutation")
