...
`ready-for-agent` is the execution gate for Software Project. `plan` owns applying
the exact label after hardening the implementation issue; `implement` owns
reading and enforcing the gate; GitStack owns GitHub transport and
...
2. Read the complete authoritative Feature Spec and implementation issue graph
   through the GitStack issue workflow, with complete pagination and no
   mutation fields.
