...
| `scope_repair_task_permission` | `granted`, `denied` | Permit a separate visible Software Project Feature task only when an active worker later needs a monotonic `allowed_paths` repair. |
...
> May I create one separate visible Software Project Feature task later, only if an
