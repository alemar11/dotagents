...
- Every Idea has exactly one tracker-owning repository. Use globally
  ...
  URL.
- The Software Project workflow contract owns the `idea` artifact marker and its
...
| --- | --- | --- |
| Software Project workflow contract | Idea reads or writes Idea metadata. | Load the exact `idea` contract value; Idea owns when it is applied, and never edits the contract at runtime. |
