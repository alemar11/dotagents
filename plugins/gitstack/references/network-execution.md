...
package registry. It is the canonical GitStack contract for shell network
...
| Network-bearing shell command | Use the runtime's scoped network permission or sandbox-escalation control from the outset for the exact command. This includes `gh`; remote `git` operations such as fetch, pull, push, and `ls-remote`; registry checks or publication; and any `scripts/gitstack` subcommand that invokes `gh`. |
...
`github.com`. The shipped GitStack CLI does not and cannot elevate itself.
...
- For authentication proof, run `<plugin-root>/scripts/gitstack --json doctor`
