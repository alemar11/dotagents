...
def _status_error(status: dict[str, Any]) -> GitStackError:
    state = status.get("status")
    if state == "gh-missing":
        return GitStackError(
    ...
    return GitStackError(
        f"The '{EXTENSION_REPOSITORY}' extension is not installed. Run 'gitstack stack ensure --install'.",
...
def _validate_noninteractive(command: str, args: Sequence[str]) -> None:
    if command in BLOCKED_COMMANDS or any(
        ...
        raise GitStackError(
            f"The 'gh stack {command}' interactive path is not available through GitStack.",
