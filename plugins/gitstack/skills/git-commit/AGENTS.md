...
the shared GitStack options reference.

## Owned surfaces

- `scripts/replace-amend-fixup-message` and
  `scripts/validate-fixup-target` are skill-local Python adapters. They are not
  alternate GitStack runtimes and must remain noninteractive and target-safe.
- Commit transport and operation journaling belong to the shared
  `plugins/gitstack/scripts/gitstack` artifact and its maintenance project.
