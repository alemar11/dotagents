...
def initialize_repository(repository: Path) -> None:
    ...
    git(repository, "config", "user.name", "GitStack Tests")
    git(repository, "config", "user.email", "gitstack-tests@example.invalid")
    ...
    def test_replace_preserves_matcher_and_replaces_message(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = run_command(
                ...
                cwd=root,
                environment={
                    "GITSTACK_AMEND_MESSAGE_FILE": str(replacement_path),
                },
            )
