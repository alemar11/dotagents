---
name: git-commit
description: Create or push Git commits with explicit staging. Use for regular commits, targeted fixups, amend-fixups, push-only requests, or commit-and-push; use $gitstack:send when the user also wants a pull request.
---
...
`$gitstack:send` instead. `commit_operation=push-only` never creates a commit;
...
If the request expands into branch publication or PR creation, route to `$gitstack:send`
