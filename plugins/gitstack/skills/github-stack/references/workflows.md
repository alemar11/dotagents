...
```bash
<plugin-root>/scripts/gitstack --json stack <command> [args...]
```
...
```bash
<plugin-root>/scripts/gitstack --json stack init --base main api-layer ui-layer
<plugin-root>/scripts/gitstack --json stack view --json
```
...
```bash
<plugin-root>/scripts/gitstack --json stack checkout <stack-number-or-pr-or-url>
```
...
```bash
<plugin-root>/scripts/gitstack --json stack link <bottom-pr> <top-pr>
```
...
```bash
<plugin-root>/scripts/gitstack --json stack submit --auto
```
...
Do not use this path for a normal `$gitstack:send` request. It does not create
...
```bash
<plugin-root>/scripts/gitstack --json stack sync
```
...
```bash
<plugin-root>/scripts/gitstack --json stack rebase
<plugin-root>/scripts/gitstack --json stack rebase --upstack
```
...
```bash
<plugin-root>/scripts/gitstack --json stack rebase --continue
```

If resolution is not safe or the user withdraws the operation:

```bash
<plugin-root>/scripts/gitstack --json stack rebase --abort
```
...
```bash
<plugin-root>/scripts/gitstack --json stack merge <pr-or-stack-number> --yes
```
