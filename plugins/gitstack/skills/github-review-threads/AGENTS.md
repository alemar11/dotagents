...
- `references/script-summary.md` mirrors the typed review commands and JSON
  envelopes exposed by the shared `scripts/gitstack` artifact. Update it with
...
- Shared provider journaling, reservation, recovery, and terminal-evidence
  logic belongs to `projects/gitstack/` and its project-scoped maintenance
  guide.
...
- Validate review contract changes with the shared GitStack tests and shipped
