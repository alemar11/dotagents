...
Use this skill only when the user invokes `$se:idea`, asks to run Idea, or a
...
- Every Idea has exactly one tracker-owning repository. Use globally
  ...
  URL.
- The SE workflow contract owns the `idea` artifact marker and its
...
writes use `$g:github-issues`. When Codex provides `request_user_input`,
...
| --- | --- | --- |
| SE workflow contract | Idea reads or writes Idea metadata. | Load the exact `idea` contract value; Idea owns when it is applied, and never edits the contract at runtime. |
| `$g:github-issues` | Idea needs exact issue or label reads, or `run_mode=publish` authorizes publication. | Pure preflight reads are allowed in either write mode and omit mutation fields. For writes, translate each operation to G-owned `mutation_mode=apply`, the exact target, and one canonical `issue_operation`. G owns safe transport, label administration, issue creation, verification, and partial recovery. |
...
Perform these preflight reads through `$g:github-issues` without mutation
...
non-reused Idea through `$g:github-issues`, checkpointing and verifying
