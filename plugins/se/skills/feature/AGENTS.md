...
`plugins/se/skills/feature/` is the single public planning surface for converging a
...
- GitHub issue transport is delegated to `$g:github-issues`. Learn owns
...
- Accepted durable decisions are carried as optional `knowledge_delta` phase
  ...
  `$se:learn` after integrated behavior is proven.
