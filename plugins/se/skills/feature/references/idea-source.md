...
Perform GitHub source reads through `$g:github-issues` in both planning
...
| `run_mode=preview` | Report `intended_coverage`, `intended_covered_scope`, `intended_remaining_scope`, and intended transitions only; leave every selected Idea unchanged, leave durable coverage unchanged and request no G mutation. |
...
`$g:github-issues` before mutating any affected Idea:
...
`$g:github-issues` for each exact operation, verify state after every
