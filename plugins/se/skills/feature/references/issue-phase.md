...
operations through `$g:github-issues` in either planning mode, with mutation
...
prove all-state enumeration and pagination completeness, require G's
read-only gap fallback to use paginated `gh api` reads. A fixed-limit
`gh issue list` result is never completeness proof. If G cannot prove
...
5. Require `$se:learn domain-memory` with
...
planning blocker through the Feature profile in the SE clarification
...
- `run_mode=publish`, GitHub: retain exact existing issues and publish only
  ...
  `$g:github-issues`. Translate each write to G-owned
  ...
  through the matching canonical G operation, verify every mutation,
...
`run_mode=preview` never invokes G for publication or mutation. It may
use pure read operations with mutation fields omitted to prove current hosted
state and convergence safety. G does not interpret Feature's tracker
or write policy.
