...
invocation contains the exact `contract_repair_request` accepted by
`contract-repair.md`. For that one exception, delegate request validation,
semantic correction, mutation ordering, audit, recovery, and result to that
reference and preserve executor-owned progress and evidence.

Without `contract_repair_request`, carry the
...
- `run_mode=publish`, GitHub: for a single Spec, publish the final sanitized
  ...
  G-owned `mutation_mode=apply`, its exact target, and one canonical
...
`run_mode=preview` never invokes G for publication or mutation. Exact
GitHub Idea discovery and source validation may still use read-only G
operations with mutation fields omitted. G does not interpret Feature
