...
- In orchestrator workspace mode, keep setup config-only: configure `AGENTS.md`, `project-memory/config/*`, root `CONTEXT.md`, and ADR layout, but do not create `projects/<project>/` or `features/<feature>/` folders until `$plan-feature` writes a real feature. Config-only means "no project or feature artifacts during setup," not "only `project-memory/config/*`."
...
- Keep Feature Spec vocabulary as a hard runtime cut with no read aliases for retired planning names, fields, modes, paths, or authority values. Before installing or distributing a runtime that contains the cut, migrate every active orchestrator ledger and active hosted or local issue body to the canonical Feature Spec contract while preserving external ids, titles, branches, links, and evidence; do not switch runtimes until a stale-vocabulary scan is clean. (Codex learning)
...
- Feature Specs must include a `Delivery Mode` section using canonical lower-kebab values: `pull-request` by default and `direct-commit` only with explicit authorization. For multi-repo `pull-request` delivery, every involved repo must use the same branch name and open its own PR. (Codex learning)
...
- Generated implementation issues inherit full delivery mode details from their `source_spec_ref`; issue bodies should still copy the effective `delivery_mode` and `project_topology` values as feature-level inherited metadata, then record parallelization, dependency ids, closeout, and only issue-level delivery or integration exceptions so orchestrator workers can run slices without inventing topology, branch, or PR strategy. (Codex learning)
...
- In local markdown mode, persist `issue_type`, `workflow_state`, and
  `source_spec_ref`; accept `Type:`, `Status:`, and `State:` only
...
- Keep final GitHub parent Feature Spec closeout root-owned: Plan Feature defines child issue closeout, while Codex Orchestrator may add the parent closing keyword to the default-branch whole Feature Spec closeout PR only after its default-required Codex review or exact owner-scoped review skip and all other Feature Spec closeout gates pass; non-default-base PRs must retain a linked later closeout vehicle, and an armed unmerged PR requires root monitoring only for a root-controlled authorized merge, otherwise a durable owner pre-merge or authorized event-driven-automation handoff, until actual parent closure is verified. (Codex learning)
