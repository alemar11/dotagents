...
- The root coordinates visible Codex task scheduling, its one Goal, Feature Spec claims, crash-safe task-operation reconciliation, coarse status, and read-only final verification; one worker per implementation-eligible Feature Spec owns implementation through delivery-ready evidence end to end. Root never edits or judges tracker criteria. Multi-repository workers receive exact peer task and checkout identities, communicate directly, and assign combined proof to existing workers; no dedicated integration worker exists. Each worker stays isolated to its own worktree, starts and cleans up its own component, and supplies exact pre/post HEAD plus endpoint evidence to the peer that owns combined proof. Cross-worktree access is forbidden. Monorepos normally integrate inside one worker/worktree. (Codex learning)
- Keep `skills/implement-feature/scripts/run-state` as the sole stateful helper at CLI version `1.0.0` and schema `1`. All controllers for one user share `~/.cache/dotagents/skills/implement-feature/run-state.sqlite3`; SQLite transactions with a fixed bounded busy timeout are the only writer lock. The fresh design has no migrations, aliases, compatibility readers, or lock file. Keep `scripts/verify-ready` read-only and outside run-state persistence. (Codex learning)
