...
Feature Specs, implementation issues, and Ideas live in GitHub Issues. Resolve
the target repository from the current Git remote.
...
Feature-workflow artifact markers, issue types, workflow states, and exact
GitHub values live in `skills/github-workflow-contract/` and are consumed by
the feature skills that read or write them. Project Context does not own that
metadata.
...
- Current Codex-dependent skills are `codex-changelog`, `code-wiki`, `focus-task`, `maintainer`, `implement-feature`, and `skill-audit`.
...
- Treat `grill-me-with-context` as portable and skill-composed because it requires `$grill-me` and `$project-context`, both portable, and otherwise relies on local repo/docs inspection.
...
- Treat `project-context` as Codex-aware but portable because session-history bootstrap, transcript lookup, and Code Review Rules history mining are optional accelerators; its core setup, capture, compaction, review-rule, and domain-modeling flows fall back to repository evidence and generic filesystem/process inspection.
...
- Treat `plan-feature` as Codex-dependent and skill-composed because its GitHub issue reads and writes require `$gitstack:github-issues`, while project context uses `$project-context`, clarification uses `$grill-me-with-context`, and issue hardening uses `$plan-harder`.
...
- Keep this registry of skills that explicitly select a delegated model: `implement-feature` -> `gpt-5.6-sol` (`medium` routine, `high` complex, `xhigh` risky/cross-system). Exclude delegations that inherit the parent or host default or are provider-managed. (Codex learning)
...
### Grill and Project Context Composition
...
- Keep `grill-me-with-context` as the thin composition layer over `grill-me` and `$project-context domain-memory`, not a duplicate questioning or domain-capture loop.
...
- Use `project-context/` as the visible root for durable project context owned by these runtime skills: flat topic files plus `project-context/adr/` for durable decision records. Use root `CONTEXT.md` as the single context entry point and route optional scoped `CONTEXT.md` files from its `Scoped Contexts` table.

### Project Context Skill

- Keep `project-context` as the normal public lifecycle surface for context setup, confirmed durable capture, explicit AGENTS.md compaction proposals, exact `## Code Review Rules`, root/scoped `CONTEXT.md`, optional `TRANSLATION.md`, flat topic files, and ADR routing or content in Git repositories.
- Use exactly one `project-context/` directory per Git repository root. Internal monorepo scopes use scoped `CONTEXT.md` files and optional flat topic files plus the root `project-context/adr/`; they must not create nested `project-context/` directories. Non-Git roots never own Project Context.
- For context discovery, treat the current Git repository as the default selected root. Explicit user scope or a validated linked Feature Spec Set authorizes the selected repository identities, but the composed caller must supply candidate local Git roots separately and verify each root against one authorized identity. Project Context never fabricates paths from Spec refs or infers scope from the ChatGPT App primary project, saved-project list, common parent, or path proximity. Read each verified root `CONTEXT.md` when present, then follow every non-overlapping `Scoped Contexts` row matched by affected paths. Reject extra or unmatched roots and inspect matched paths without context directly rather than creating dangling pointers. (Codex learning)
- For implementation closeout that carries accepted durable decisions, require the implementor to invoke `$project-context domain-memory`; Project Context must run its internal domain-modeling workflow, reconcile the carried delta against behavior that actually landed, update only the named durable surfaces, and verify the documentation diff. `$plan-feature` assigns this work but must not perform it during planning. (Codex learning)
- Keep `skills/project-context/references/options.md` limited to the genuine controls `memory_slice`, `domain_operation`, and `capture_mode`. Derive execution context from repository evidence, treat a knowledge handoff as input data, and report capture outcome as result state rather than selectable configuration. (Codex learning)
- Always use `AGENTS.md` for short project-context pointers when an agent-instruction file is needed; keep shared context/index data in `CONTEXT.md`, conditional detail in indexed topic files, and accepted decisions in ADRs.
- Keep `github-workflow-contract` as the transitional reusable owner of feature-workflow artifact markers, issue types, workflow states, and exact GitHub values. Its contract is consumed by the skills that actually read or write those values; GitStack owns transport and mutation safety, while feature workflows resolve their repository target from the current Git remote.
...
- Do not add schema versions to `project-context` generated Markdown files or templates unless a concrete parser or migration workflow requires versioning.
- Keep behavior-affecting contract tables human-first but table-first. Leave prose for rationale and conventions; keep repository identity as runtime evidence rather than Project Context configuration.
- Keep setup conservative: only bootstrap domain memory for existing projects when the evidence is accepted, load-bearing, and not merely tentative session discussion.
- Keep first-time-user setup ambiguity prompts canonical in `skills/project-context/references/setup-questions.md`: normally ask no questions, show concrete conflicting evidence when one is necessary, use project language rather than Project Context internals, and translate answers to canonical context data internally.
- Keep Code Review Rules inside Project Context as `memory_slice=code-review-rules`; load the copied rule-evidence and evaluation references, inspect the closest applicable `AGENTS.md` chain, and write only the exact invariant, consequence, and safe path under `## Code Review Rules`. Keep matrices, provenance, and history coverage out of `AGENTS.md`. (Codex learning)
- During authorized Project Context domain bootstrap, always create or update root `CONTEXT.md` at every selected Git root. Keep evidence-poor roots minimal with explicit unknowns; use verified monorepo scopes for stable routing, and require stronger accepted evidence before adding richer vocabulary, rules, boundaries, or scoped context files.
- An explicit Project Context setup or update request authorizes writes to the selected context surfaces; direct durable capture and AGENTS compaction remain proposal-first, while composed inline capture requires named accepted knowledge and targets.
- Keep `project-context` durable capture manual and conservative: show the exact target and wording, wait for affirmative confirmation, preserve unrelated text, and suffix inserted AGENTS learning bullets with ` (Codex learning)`.
- When durable guidance scope is unclear, ask where to save it and recommend the narrowest applicable `AGENTS.md`; never fall back to global because a project target is missing.

### GitHub Workflow Contract Skill

- Keep `skills/github-workflow-contract/` as the transitional standalone home for the feature metadata contract while the future `feature-flow` plugin is designed. Preserve its explicit boundary: it defines values and ownership, but GitStack performs reads and writes.
- Do not restore feature label or issue-type mappings to Project Context. A future plugin migration may move this contract into its bundled skill tree without changing the consuming vocabulary.
...
- Keep `skills/capture-idea/references/options.md` limited to the run-scoped `write_mode` control. Consume the feature metadata contract, tracker owner, candidate decisions, queue intent, and refs as execution facts or data.
- Keep one durable GitHub Idea per independently plannable proposal and exactly one tracker-owning repository per Idea. Ideas use the `github-workflow-contract` `idea` label with native Issue Type unset and optional explicit `needs-triage` only at capture.
- Keep Capture Idea separate from Feature Spec drafting and domain memory. `$plan-feature` owns durable Idea-source consumption and lifecycle reconciliation; `github-workflow-contract` owns feature metadata semantics; GitStack owns GitHub mutation mechanics.
...
- Keep `skills/plan-feature/references/publication.md` as the owner of GitHub publication, stable hosted refs, and recoverable publication transactions; consuming phases own only their branch-specific behavior.
- Keep `skills/plan-feature/references/options.md` as the owner of the sole default-path control, `planning_mode`. Consume the resolved GitHub target and derive the affected repository set and source route from explicit intake evidence; do not add provider, delivery, or repository topology as a run option or durable config. (Codex learning)
- Keep GitHub Issues and pull-request delivery fixed for Plan Feature. Do not render provider or delivery selectors in Feature Specs or implementation issues; executor branch and merge behavior remains outside planning. (Codex learning)
...
- Keep Feature Spec vocabulary and the single convergent pipeline as a hard runtime cut with no read aliases for retired planning names, fields, values, paths, or authority values. Active planning state and hosted issue bodies must already use the canonical Feature Spec contract before the runtime consumes them; do not switch runtimes until a stale-vocabulary scan is clean. (Codex learning)
- `planning_mode=publish` publishes through GitHub; `planning_mode=preview` performs no writes and returns proposed bodies, repository targets, metadata, and publication order. Never publish incomplete Feature Specs or mark partial work agent-ready; uniquely marked non-executable staging issues are permitted only for hosted roles inside the recoverable multi-repository publication transaction. (Codex learning)
...
- For recoverable hosted publication, persist reconstructable typed transaction facts and compare stable fields directly while preserving mutable execution content. Do not use tracker-text, template, body, result, packet, or message hashes. Resume only a recognized transaction, create or finalize only its exact missing targets and relationships, and treat every other appeared or materially changed target as a race. (Codex learning)
- Keep the `plan-feature` issue phase focused on converging Feature Specs into vertical GitHub issues. Enumerate complete durable state before synthesis, seed the candidate graph with contract-equivalent issues as fixed IDs/slices, synthesize only uncovered scope, create only missing issues, repair only missing feature-contract metadata or parent/sub-issue attachment, return a verified no-op for a complete bundle, and stop on stale, conflicting, duplicate, or extra artifacts. Never renumber or regenerate a retained issue. Re-read the owning Spec, the `github-workflow-contract`, and complete issue/metadata/relationship state immediately before proposal, no-op, or the first mutation and prove exact absence before every create. Dependency data and source-body sibling or cross-Spec relationships must already match. After graph and scope stabilization it must run `$plan-harder` one or more times per missing issue, rerun it after material pre-publication repairs, and persist only the final stable brief plus one provenance line without duplicating top-level sections.
- Keep Plan Feature GitHub-only: hosted issues are authoritative, body files are transient transport outside the repository, and proposal output never creates a mirror. Feature Spec and issue acceptance criteria are separate stable contracts and need not be textually identical; require a transient complete Spec-to-issue coverage map before publication. (Codex learning)
- Generated implementation issues are the durable execution graph by default; do not create a separate hosted issue, Feature Spec section, or inline scheduling artifact. If a user asks for a summary, return it as a non-authoritative view derived from current issue bodies. (Codex learning)
...
- Treat Plan Feature `allowed_paths` as the smallest complete and reasonably predictable execution envelope, not the shortest guessed file list: prefer stable directory or subsystem prefixes, include foreseeable feature-owned implementation, integration, validation, test, fixture, configuration, generated-contract input, and technical-documentation paths, and never under-scope merely to manufacture disjoint scheduling. Keep genuine single-file boundaries exact, exclude unrelated pre-existing failures, and require evidence before authorizing a repository-wide wildcard. Keep the canonical machine-readable rule table in `skills/plan-feature/references/spec-phase.md`. (Codex learning)
...
- Keep generated issue type/state in the GitHub tracker surface: consume the explicit `github-workflow-contract` transport and exact value, reject missing or incompatible contract data, and create and verify only exact missing labels before dependent published mutations; preview mode reports label-creation intent only. GitHub native Issue Types are not part of this repository's feature-workflow contract. (Codex learning)
...
- GitHub implementation issue headings use `<feature-slug>: <NN> <vertical outcome>`.
- In GitHub issue-tracker mode, Feature Spec issues use the contract's `enhancement` label and generated implementation sub-issues use the contract's `task` label; workflow readiness uses the contract's `ready-for-agent` label.
...
- Keep every applied multi-repository source and dependency ref globally unambiguous with `owner/repository#<number>` or a canonical hosted URL. A linked member's lower-kebab `repository_key` is stable, unique inside the frozen set, at most 48 characters, and persisted in Planning Identity. Propagate the same identity through every `Feature Spec Set`, Feature Dependency, and issue `source_spec_ref`; proposed refs use `proposed-spec:<feature-id>/<repository-key>` and never become executable. (Codex learning)
...
- Keep bundled skills provider-primitive and workflow-agnostic: caller-specific planning, orchestration, project-context, queue-state, issue-body, and label-taxonomy policy belongs in the composing skill.
...
- Keep `skills/implement-feature/scripts/run-state` as the sole stateful helper. Version its CLI, runtime contract, and named JSON protocols independently with SemVer; keep the SQLite database schema as a separate monotonically increasing integer. The current runtime line is CLI `7.0.0`, runtime contract `9.0.0`, CLI-envelope `3.0.0`, app-operation observation `4.0.0`, run-manifest `4.0.0`, Feature Spec Set input `2.0.0`, delivery-ready and recovery observations `3.0.0`, assignment-resume `2.0.0`, scope-repair `1.0.0`, and database schema `6`. Protocol payloads must carry exact `schema` and `schema_version` fields, reject aliases and unknown keys, and take a major protocol bump for incompatible key, type, or enum changes. Keep `scripts/verify-ready` read-only and outside run-state persistence. (Codex learning)
- Create each ordinary worker task with its complete canonical title in the initial `create_thread` call, then reconcile task identity, checkout, lifecycle, and exact title together in the single `create-worker` observation. On title drift, preserve the truthful successful creation receipt and binding, forbid bootstrap, and require pre-bootstrap archival; never add a follow-up worker-title mutation. (Codex learning)
- All controllers for one user share the permanently unversioned `~/.cache/dotagents/skills/implement-feature/run-state.sqlite3`; the single-row application-owned `runtime_metadata` table is the sole database-schema source of truth. Never use `PRAGMA user_version`, an external lock file, migrations, data-copy upgrades, importers, compatibility state files, or versioned DB filenames. Every run pins the exact runtime contract, CLI version, and shipped artifact SHA-256, and only that exact runtime may mutate it. Any future database-schema or SQLite-shape change is a breaking hard cut and requires explicit user consent before implementation. (Codex learning)
- A consented SQLite-shape change is a breaking hard cut: move the current schema integer to `6`, create only the complete current schema, and reject every other schema number or shape. Do not track historical schema registries, target-schema fences, retained-runtime drains, migrations, data-copy upgrades, importers, versioned DB filenames, or automatic resets. Existing non-current, unversioned, corrupt, and same-number-invalid databases fail closed without deletion or row carry-forward. Keep the executable hard-cut contract in `skills/implement-feature/references/run-state.md`. (Codex learning)
- Treat every Codex task mutation as one generated logical `operation_id`. For bootstrap authority, derive and propagate one stable `bootstrap_id`, require worker-side deduplication, and replay only the same reconciled bootstrap identity after authoritative readback; this guarantees an exactly-once effect, not exactly-once delivery. Scope revision similarly preserves one repair ID, derived revision ID, and target contract generation across replay. Native `codex review` is always worker-owned; root only orchestrates and verifies returned evidence. Each authorized launch carries a monotonic `launch_count`; reject observations from stale generations, and allow protected non-bootstrap replay only after authoritative failed readback proves no effect, except deduplicated scope revision may replay after authoritative unknown or failed readback. Keep observation templates as descriptors and observation builders as pure, strict, no-overwrite artifact writers; only the corresponding finish or ready command may mutate run state. (Codex learning)
- Keep state limited to runs, normalized per-run repository-to-project bindings, assignments, validated linked `feature_id` membership, canonical GitHub Feature Spec claims, contract generation, opaque scope repair identity/readback, normal Git head/base facts, and typed Codex task-operation reconciliation facts; PR/provider refs are intrinsic to the GitHub delivery. Each affected repository maps to one distinct local saved Git project, assignments inherit that mapping, and a saved project may not stand in for multiple repositories in one run. Validate complete ephemeral linked member bodies before state, then persist only the validator-produced UUID membership: do not persist raw Spec/issue bodies, requested or allowed paths, normalized Feature Spec Set tables, responsibility or criterion text, checklists, issue phases, validation attempts, process/port state, worker technical/domain state, generic request/result payloads, or text hashes. Reject stale or retired DB structure exactly. (Codex learning)
- GitHub Issues is the only source tracker and GitHub PR is the only delivery boundary for Implement Feature. Manifests, claim/recovery observations, and ready observations must not expose provider or delivery selectors; runtime rejects local repository identities, local source refs, local branches, and mixed vectors. The visible terminal status is `pr-ready-for-merge`, and the run outcome is `pr-ready`; merge and post-merge work remain outside the skill. (Codex learning)
- Preserve the core runtime boundaries: one startup authorization interaction, exact one-to-one local saved Git-project preflight for every affected repository before state, explicit and separate authority for any missing-project creation, path- and dependency-safe scheduling without a fixed numeric worker cap, per-assignment Spec/head-branch claims, crash-safe Codex task changes, evidence-only root follow-ups, worker-owned implementation and review, worker-owned repair, and declarative durable-drift blocking. On one implementation-time path miss, retain the same worker/task/worktree/bootstrap and claim, delegate only the portable monotonic scope mutation to a separate Plan Feature task, recompute whole-envelope overlap without file claims, and let root deliver generation `N+1`; a second miss requires full replanning. Remote, non-Git, broad parent, duplicate, or ambiguous saved projects never substitute for an exact affected-repository project. Keep executable detail only in `skills/implement-feature/SKILL.md` and its references. (Codex learning)
