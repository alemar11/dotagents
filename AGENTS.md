...
- Treat `plan-feature` as portable and skill-composed because it requires `$setup-project-memory`, `$grill-me-with-context`, and `$plan-harder`, uses `$github-issues` for GitHub tracker publishing, and otherwise relies on internal PRD and issue phases plus project-memory routing rather than Codex-only runtime tools.
