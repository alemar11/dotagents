...
- Name embedded CLIs independently from their hosting skills by default: the skill name is the guidance/package container, while the CLI/tool name owns `scripts/<tool>` and `projects/<tool>/`. Reuse the skill name only when it is intentionally the clearest ecosystem-standard runtime noun. (Codex learning)
