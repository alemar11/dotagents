...
- Keep orchestration runtime policy out of project-memory setup files. `$codex-orchestrator` invocation authorizes background Codex subagents for the root and its spawned Codex App tasks; visible App tasks require `visible_app_task_permission=granted-by-authorized-user`. That grant selects mandatory one-visible-task-per-Feature-Spec execution, with count derived from eligible Specs; the orchestrator still derives recursive subagent topology, parallelism, and sequencing within the selected implementation checkout strategy. Do not store durable dispatch flags, worker-count preferences, visible-task limits, authorization ceilings, scheduled check timing, publication policy, issue mutation policy, or checkout strategy as project-memory config.
...
- Keep worker authorization surface-based: invocation authorizes background Codex subagents for the root and its spawned Codex App tasks, while visible Codex App tasks require explicit owner consent. Once granted, consent selects mandatory one-visible-task-per-Feature-Spec mode: each task owns its Spec through merge-ready PR closeout, including Codex-review polling and fixes, while the root only schedules, monitors, steers drift, and reconciles evidence and must never take implementation or review back. Each task chooses its internal subagent topology within checkout-safety limits; the root derives serial/parallel scheduling from dependencies, checkout strategy, and capacity. Cap concurrent nonterminal Feature Spec executions at three across worker surfaces, with serial caller-checkout mode further capped at one; keep that ceiling internal and do not expose delegation toggles, numeric worker-cap options, or a separate visibility selector. (Codex learning)
- Default visible Feature Spec implementation tasks to one managed worktree per Spec. An exact authorized-user instruction to avoid worktrees selects strict serial caller-checkout execution: preserve the original branch, require a clean checkout, persist run-wide ownership of one dedicated feature branch per Spec and repository, keep one task active through its complete delivery target, restore and verify the original branch, then dispatch the next Spec. Retain completed and blocked branch assignments for recovery. Never implement on the original branch, share one repository/target-branch pair across Specs, or let parallel subagents mutate the shared checkout. (Codex learning)
