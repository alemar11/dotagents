...
- Keep orchestration runtime policy out of project-memory setup files. `$codex-orchestrator` treats background Codex subagents as authorized by invocation unless the authorized user disables delegation; when visible App tasks may be used, it resolves `visible_app_task_permission` and `max_visible_app_tasks`, then chooses the worker split within `work_delegation_policy` and `max_concurrent_delegated_workers`. Do not store durable dispatch flags, visible task limits, authorization ceilings, scheduled check timing, publication policy, or issue mutation policy as project-memory config.
...
- Orchestrator reports and ledgers must record worker evidence when delegation is requested or approved: selected `delegated_worker_visibility`, `visible_app_task_permission`, `actual_execution_location`, worker/session id or failure evidence, fallback reason, and parallel/sequential/root-owned execution. (Codex learning)
