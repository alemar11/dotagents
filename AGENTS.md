...
- Keep orchestration runtime policy out of project-memory setup files. `$codex-orchestrator` requires `visible_app_task_permission=granted-by-authorized-user` plus one App-managed visible task per Feature Spec and derives internal subagents, scheduling, and the three-Spec cap. Do not store dispatch flags, worker limits, authorization ceilings, publication policy, or runtime checkout paths as project-memory config.
...
- Orchestrator reports use the Feature Spec task registry. Rows record permission, one-to-one Spec/task mapping, managed checkouts, task Goal and review ownership, drift corrections, and forbidden root implementation fallback. Parallelism and topology are derived, never user-configured. (Codex learning)
...
- Keep `skills/codex-orchestrator/references/options.md` as the complete option registry. Merge authorization and confirmation fields are invalid orchestrator inputs because the skill stops at ready-to-merge. Ledgers, prompts, handoffs, recovery packets, and reports must use the canonical App-only values. (Codex learning)
...
- Keep `skills/codex-orchestrator/scripts/orchestrator-claim` as the sole atomic active-root primitive. Its filesystem lock and claim JSON are authoritative; the Markdown ledger is a projection. The App root acquires before ledger/Goal/task/worktree creation, heartbeats while active, and releases only after terminal or durable handoff evidence. The helper has no worker-surface selector. (Codex learning)
...
- App orchestration begins by verifying that the current runtime exposes visible Codex App task creation and App-managed worktree binding. Skill discovery, filesystem access, and generic subagents are not proof of the App runtime; missing or unverifiable App surfaces abort before permission, source intake, ledger/Goal creation, or mutation. Only after that check passes, run the mandatory run-scoped visible-task permission gate. If the invoking instruction does not grant permission, ask once; denial, no answer, or inability to ask aborts. After a grant, use exactly one visible task per Feature Spec. App execution uses only App-managed worktrees; missing task or checkout capability aborts as blocked, with no caller-branch, unmanaged-worktree, or root/background implementation fallback. (Codex learning)
...
- Keep cross-Feature-Spec dependencies authored in the Feature Spec and distinct from intra-Spec implementation-issue `dependency_ids`. Default authored edges to waiting for the upstream merge. Permit `upstream-merge-ready-head` stacking only for two single-repository Specs in the same repository, with one unresolved upstream and maximum unmerged depth two: the downstream task starts from the exact reviewed upstream head, implements and validates, and publishes its pre-promotion draft against the upstream branch without final Codex review. Only after that draft state exists, emit the intermediate upstream merge handoff before waiting, keep the orchestration claim active, observe the separate GitHub workflow's merge, then reconcile the downstream onto the default branch and rerun all final gates. Never let the downstream PR close, supersede, or substitute for the upstream PR. (Codex learning)
- Assign the pre-merge parent Feature Spec closing keyword to the owning visible task. Add it only to the default-branch whole-Spec PR after its required Codex review or exact owner-scoped review skip and all other closeout gates pass; non-default-base PRs retain a linked later closeout vehicle. Record `armed` and hand the exact merge and post-merge issue-closure work to a separate GitHub workflow; the orchestrator must not watch or perform final closure after its terminal report. (Codex learning)
