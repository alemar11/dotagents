...
- Preserve the core runtime boundaries: one startup authorization interaction, exact saved Git-project preflight before state, explicit and separate authority for any missing-project creation, at most three workers, per-assignment Spec/head-branch claims, crash-safe ChatGPT task changes, evidence-only root follow-ups, worker-owned repair, and declarative durable-drift blocking. A non-Git coordination workspace never substitutes for the saved Git project needed by worktree creation. Keep executable detail only in `skills/implement-feature/SKILL.md` and its references. (Codex learning)
