...
- Keep `codex-orchestrator` as the single App-only reusable orchestration entrypoint under `skills/codex-orchestrator/`; do not add a terminal-session orchestration sibling or alternate worker surface.
- Keep `skills/codex-orchestrator/references/options.md` limited to `visible_app_task_permission` and exceptional `stale_claim_takeover_permission`. Every other behavior is a fixed invariant, execution-bundle data, or derived runtime evidence. (Codex learning)
...
- Keep `skills/codex-orchestrator/scripts/orchestrator-claim` as the sole atomic active-root primitive. Its fixed per-user claim root, filesystem lock, and claim JSON are authoritative; the Markdown ledger is a projection. Do not add adapter fields or claim-root overrides. (Codex learning)
