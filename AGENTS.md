...
- Give each implementation issue one canonical `## Execution Contract` containing its source ref, feature slug, affected repositories, allowed paths, per-Spec target branch, and `dependency_ids`. Keep one branch shared only inside each Feature Spec, require exactly one Spec owner for every implementation-bundle `(repository, target_branch_name)` pair, and derive the integration-partial branch as `<ordinary_target_branch_name>-integration` so an executor never binds two Specs to the same repo branch. Keep goal, requirements, acceptance, and validation authoritative in their normal sections; do not repeat option, delivery, or handoff projections. (Codex learning)
...
- The root coordinates visible Codex task scheduling, its one Goal, Feature Spec claims, crash-safe task-operation reconciliation, coarse status, and read-only final verification; one worker per implementation-eligible Feature Spec owns implementation through delivery-ready evidence end to end. Root never edits or judges tracker criteria. A dedicated multi-repository integration Feature Spec therefore owns another visible Codex worker; monorepos normally integrate inside one worker/worktree. (Codex learning)
- Keep `skills/implement-feature/scripts/run-state` as the sole shipped helper at CLI version `1.0.0` and schema `1`. All controllers for one user share `~/.cache/dotagents/skills/implement-feature/run-state.sqlite3`; SQLite transactions with a fixed bounded busy timeout are the only writer lock. The fresh design has no migrations, aliases, compatibility readers, or lock file. (Codex learning)
...
- Preserve the core runtime boundaries: one startup permission, at most three workers, per-assignment Spec/head-branch claims, crash-safe ChatGPT task changes, evidence-only root follow-ups, worker-owned repair, and declarative durable-drift blocking. Keep their executable detail only in `skills/implement-feature/SKILL.md` and its references. (Codex learning)
