...
- Keep `skills/implement-feature/scripts/orchestrator-claim` as the sole atomic active-root primitive. Its fixed per-user claim root, filesystem lock, and claim JSON are authoritative; the Markdown ledger is a projection. Do not add adapter fields or claim-root overrides. (Codex learning)
...
- Persist portfolio ledgers under `~/.cache/dotagents/skills/implement-feature/ledgers/`, with one ledger per named portfolio by default.
