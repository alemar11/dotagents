...
- Keep `skills/implement-feature/scripts/run-state` as the sole stateful helper at CLI version `2.0.0` and schema `2`. All controllers for one user share `~/.cache/dotagents/skills/implement-feature/run-state.sqlite3`; SQLite transactions with a fixed bounded busy timeout are the only writer lock. The hard-cut design has no migrations, aliases, compatibility readers, or lock file and rejects every v1 DB and payload without modification. Keep `scripts/verify-ready` read-only and outside run-state persistence. (Codex learning)
- Keep state limited to runs, assignments, canonical Feature Spec claims, delivery type, normal Git head/base facts, and typed ChatGPT task-operation reconciliation facts; PR/provider refs exist only for GitHub delivery. Do not persist raw Spec/issue bodies, checklists, issue phases, validation attempts, process/port state, worker technical/domain state, generic request/result payloads, or text hashes. Reject stale same-number DB structure exactly and never delete the user's DB automatically. (Codex learning)
