...
- A stale-claim takeover asks permission before task interruption, replaces only roots whose complete repository/source scope the candidate covers, and adopts the original visible tasks rather than dispatching replacements. Unrelated legacy claims must not block a current owner's acquire, heartbeat, or release.
