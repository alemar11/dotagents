...
- The root coordinates App scheduling, its one Goal, Feature Spec claims, typed App-operation reconciliation, coarse status, and read-only final verification; one App worker per Feature Spec owns implementation through PR-ready evidence end to end. Root never edits or judges tracker criteria. (Codex learning)
...
- Keep state limited to runs, assignments, canonical Feature Spec claims, and typed App-operation reconciliation facts. Do not persist raw Spec/issue bodies, checklists, issue phases, validation attempts, worker technical/domain/provider state, generic request/result payloads, or text hashes. Normal Git head SHAs remain valid evidence. (Codex learning)
- Claim each assignment by canonical repository plus canonical durable `source_spec_ref`, and keep its implementation head branch unique within that repository. Different roots may claim different Specs and head branches in one repository; acquire, wait, block, recover, and release per assignment rather than repository-wide. A post-bootstrap durable-contract block retains only its Spec claim; root-verified PR-ready evidence releases that assignment claim. (Codex learning)
- Ask exactly once at startup for visible App-task permission. The full worker assignment starts on bootstrap receipt/readback; do not add later authority, recovery, validation, or blocked-state questions. Recover exact bootstrap through App receipt/readback and direct stable-section comparison, never packet or message hashes. (Codex learning)
- Schedule at most three path-disjoint workers per root and serialize that root's overlaps and dependencies. Cross-root path overlap is ordinary Git/PR integration evidence, not a controller claim. Recover a conflicting Spec autonomously only from authoritative terminal App proof; active or ambiguous evidence retains the owner, and no TTL or heartbeat authorizes takeover. Claim release never substitutes for upstream merge and integration proof. (Codex learning)
