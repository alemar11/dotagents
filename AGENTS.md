...
- Both `plan-feature` phases should read `project-memory/config/issue-tracker.md` and related project memory before deciding where PRDs or issues belong.
...
- Keep final GitHub parent-PRD closeout root-owned: Plan Feature defines child issue closeout, while Codex Orchestrator may add the parent closing keyword to the default-branch whole-PRD closeout PR only after its default-required Codex review or exact owner-scoped review skip and all other PRD closeout gates pass; non-default-base PRs must retain a linked later closeout vehicle, and an armed unmerged PR requires root monitoring only for a root-controlled authorized merge, otherwise a durable owner pre-merge or authorized event-driven-automation handoff, until actual parent closure is verified. (Codex learning)
