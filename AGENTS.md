...
- Keep `project-memory` as the normal public lifecycle surface for creating or refreshing `AGENTS.md` pointers and the exact `## Code Review Rules` section, plus `project-memory/config/issue-tracker.md`, `project-memory/config/triage-labels.md`, root and optional scoped `CONTEXT.md`, optional `TRANSLATION.md`, and ADR routing or content in Git repositories.
...
- Keep behavior-affecting mapping files human-first but table-first, and keep the resolved GitHub target as factual prose rather than a provider option. Leave prose for rationale and conventions.
...
- Keep generated issue type/state in the GitHub tracker surface: consume the explicit Project Memory transport and exact value, using `native-type`, `label`, or `body-field` for GitHub types and `label` for GitHub workflow state. Reject missing or incompatible transports. Revalidate native-type availability and exact values before publication, and create and verify only exact missing configured labels before dependent applied mutations; proposal mode reports label-creation intent only. Never issue a native type mutation when types are disabled. (Codex learning)
