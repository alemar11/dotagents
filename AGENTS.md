...
- Keep `skills/codex-orchestrator/references/options.md` as the pre-conclusion option registry and `skills/codex-orchestrator/references/merge-authorization.md` as the separately loaded post-conclusion merge registry. The orchestrator does not load or resolve merge authorization during intake, CLAIM, registration, or worker execution. Ledgers, prompts, handoffs, recovery packets, and reports must use those canonical values. (Codex learning)
