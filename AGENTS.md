...
- Treat `plan-feature` as Codex-dependent and skill-composed because its GitHub issue reads and writes require `$gitstack:github-issues`, alongside `$project-memory`, `$grill-me-with-context`, and `$plan-harder`.
...
- Keep `project-memory` as the normal public lifecycle surface for creating or refreshing `AGENTS.md` pointers and the exact `## Code Review Rules` section, plus `project-memory/config/issue-tracker.md`, root and optional scoped `CONTEXT.md`, optional `TRANSLATION.md`, and ADR routing or content in Git repositories.
...
- Keep issue-tracker setup limited to the resolved GitHub target and human-readable conventions; do not persist a provider selector. Delivery target, branch, PR shape, publication mode, issue-mutation authority, repository-set topology, and runtime orchestration policy are not Project Memory configuration. (Codex learning)
...
- Keep behavior-affecting contract tables human-first but table-first, and keep the resolved GitHub target as factual prose rather than a provider option. Leave prose for rationale and conventions.
- Keep setup conservative: it configures tracker locations and conventions for fresh projects, and only bootstraps domain memory for existing projects when the evidence is accepted, load-bearing, and not merely tentative session discussion.
...
- Both `plan-feature` phases should read each selected repository's `project-memory/config/issue-tracker.md`, the `github-workflow-contract`, and applicable context before deciding where its Feature Spec or issues belong.
