...
- Current Codex-dependent skills are `autoreview`, `codex-changelog`, `code-wiki`, `learn`, `maintainer`, `codex-orchestrator`, `codex-cli-orchestrator`, and `skill-audit`.
...
- Treat `codex-orchestrator` as Codex-dependent and runtime-dependent on `python3` plus local `git` because it is the App-only adapter: implementation requires the shipped atomic claim helper, explicitly authorized visible Codex App tasks, task-owned Goals, App-managed worktrees, execution-ready Feature Spec bundles, shared ledgers under `~/.cache/dotagents/skills/codex-orchestrator/ledgers/`, `$autoreview`, and GitStack workflows. It has no planning, root/background implementation, raw Git worktree, or tmux machinery.
- Treat `codex-cli-orchestrator` as Codex-dependent and runtime-dependent on `python3`, local `git`, `ps`, `tmux`, and Codex CLI. It requires the sibling `codex-orchestrator` shared references and atomic claim helper, uses `skills/codex-cli-orchestrator/scripts/codex-session`, stores disposable run evidence under `~/.cache/dotagents/skills/codex-cli-orchestrator/runs/`, and never invokes visible App task tools.
...
- Keep orchestration runtime policy out of project-memory setup files. `$codex-orchestrator` is the App adapter and requires `visible_app_task_permission=granted-by-authorized-user` plus one App-managed visible task per Feature Spec. `$codex-cli-orchestrator` keeps the current CLI session as root and uses one tmux-hosted Codex session per Feature Spec. Both derive internal subagents, scheduling, and the shared three-Spec cap; do not store adapter selection, dispatch flags, worker limits, authorization ceilings, publication policy, or runtime checkout paths as project-memory config.
...
- Orchestrator reports use the shared Feature Spec execution registry plus adapter evidence. App rows record permission, one-to-one Spec/task mapping, managed checkouts, task Goal and review ownership, drift corrections, and forbidden root implementation fallback. CLI rows record run/spec ids, tmux identity, Codex session UUID, manual worktrees, structured artifacts, internal subagents, and root-owned delivery. Parallelism and topology are derived, never user-configured. (Codex learning)
...
- Keep `codex-orchestrator` as the App-only reusable orchestration entrypoint under `skills/codex-orchestrator/`; keep `codex-cli-orchestrator` as the CLI-only sibling under `skills/codex-cli-orchestrator/`. The CLI package may load only canonical shared references from the App package and must never invoke its App entrypoint.
- Keep `skills/codex-orchestrator/references/core/options.md` as the shared non-merge authority and delivery registry, `skills/codex-orchestrator/references/core/merge-authorization.md` as the post-conclusion merge registry, and `skills/codex-orchestrator/references/options.md` as the App-only registry. Neither adapter loads or resolves merge authorization during intake, CLAIM, registration, or worker execution. Ledgers, prompts, handoffs, recovery packets, and reports must use those canonical values. (Codex learning)
...
- Treat one active App or CLI orchestrator root as the owner for a project or portfolio source graph. Both adapters share the same active-root ledger claim namespace; overlapping roots must stop or perform an explicit takeover/handoff.
- Keep `skills/codex-orchestrator/scripts/orchestrator-claim` as the sole atomic active-root primitive shared by App and CLI adapters. Its filesystem lock and claim JSON are authoritative; the Markdown ledger is a projection. Both adapters acquire before ledger/Goal/run/task/worktree creation, heartbeat while active, and release only after terminal or durable handoff evidence. (Codex learning)
...
- App orchestration begins by verifying that the current runtime exposes visible Codex App task creation and App-managed worktree binding. Skill discovery, CLI execution, filesystem access, and generic subagents are not proof of the App runtime; missing or unverifiable App surfaces abort before permission, source intake, ledger/Goal creation, or mutation. Only after that check passes, run the mandatory run-scoped visible-task permission gate. If the invoking instruction does not grant permission, ask once; denial, no answer, or inability to ask aborts. After a grant, use exactly one visible task per Feature Spec. The App skill never invokes, routes to, or recommends the CLI skill. App execution uses only App-managed worktrees; missing task or checkout capability aborts as blocked, with no caller-branch, unmanaged-worktree, or root/background implementation fallback. (Codex learning)
- After App task permission, require one read-only execution-ready intake before the active-root claim. Accept only durable Feature Specs with complete generated implementation issues and Orchestrator Handoffs; reject rough intent, standalone Feature Specs, draft refs, and ad-hoc implementation requests. Missing or contradictory evidence returns `planning-required` before ledger, Goal, task, tracker, or source mutation. The App orchestrator must never invoke a planning skill, regenerate planning artifacts, or expose an automatic-planning option. (Codex learning)
- Keep `pull-request-ready-for-merge-but-not-merged` as the App orchestrator's only successful implementation conclusion. It is a fixed invariant, not a user knob: draft PR, uncommitted, commit-only, and push-only states are intermediate or unsupported. After spawn permission and source registration, require GitHub publication/review/CI capability for every affected repository; failure aborts as blocked without target downgrade. Multi-repo App completion requires one ready-to-merge PR per affected repository. Merge remains separately authorized and root-owned. (Codex learning)
- CLI orchestration keeps the interactive CLI session as root and hosts one persistent `codex exec` worker per Feature Spec in tmux, up to the shared cap of three. Workers edit, validate, and report from unique manual worktrees; the root owns commits, publication, review, and closeout. Monitor structured artifacts and session UUIDs, never pane scraping or `tmux send-keys`. (Codex learning)
- Require every orchestrator-created visible Codex App task to establish or resume its own assignment-scoped Goal before work begins. The root supplies the objective and terminal delivery target, verifies task-reported Goal evidence, and records an exact objective fallback only when that task runtime exposes no Goal tool; this is automatic runtime behavior, not a user option, and does not apply to internal background subagents. Treat user mentions of Codex threads as visible Codex App tasks unless they explicitly refer to API/protocol objects or GitHub PR review threads; keep API tool names at the technical boundary. (Codex learning)
...
- For `pull-request-ready-for-merge-but-not-merged`, keep a newly published PR draft through required exact-revision Codex review, feedback disposition, fix validation, and current CI; review freshness includes head SHA, base ref, and merge-base SHA. Mark it ready only after that revision has a verified terminal review with no unresolved actionable feedback, or an exact owner-scoped review skip plus all remaining gates pass. (Codex learning)
- Keep cross-Feature-Spec dependencies authored in the Feature Spec and distinct from intra-Spec implementation-issue `dependency_ids`. Default authored edges to waiting for the upstream merge. Permit `upstream-merge-ready-head` stacking only for two single-repository Specs in the same repository, with one unresolved upstream and maximum unmerged depth two: the downstream task starts from the exact reviewed upstream head, keeps its PR draft against the upstream branch without final Codex review, waits for the upstream PR to merge independently, then reconciles onto the default branch and reruns all final gates. Never let the downstream PR close, supersede, or substitute for the upstream PR. (Codex learning)
- Keep final GitHub parent Feature Spec lifecycle closeout root-owned, but assign the pre-merge PR-body closer to the owning visible Feature Spec task when mandatory task mode is active. The closer may be added to the default-branch whole-Spec PR only after its default-required Codex review or exact owner-scoped review skip and all other Feature Spec closeout gates pass; non-default-base PRs retain a linked later closeout vehicle. After `armed`, the root owns any authorized merge watch and post-merge issue-closure verification, or establishes the durable owner/event-driven-automation handoff. (Codex learning)
