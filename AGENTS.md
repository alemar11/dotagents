...
- Treat `plan-feature` as portable and skill-composed because it requires `$project-memory`, `$grill-me-with-context`, and `$plan-harder`, uses `$github-issues` for GitHub tracker publishing, and otherwise relies on internal PRD and issue phases plus project-memory routing rather than Codex-only runtime tools.
- Treat `triage` as portable and skill-composed because it relies on project-memory issue-tracker mappings, can load `$project-memory` when setup is missing, uses `$grill-me-with-context` for underspecified repo-backed issue intent, requires `$plan-harder` before marking an issue `ready-for-agent`, and uses `$github-issues` for GitHub issue mutations.
...
- Keep `project-memory` as the reusable setup surface for creating or refreshing `AGENTS.md` pointers plus `project-memory/agents/issue-tracker.md`, `project-memory/agents/triage-labels.md`, `project-memory/agents/domain.md`, optional `project-memory/agents/orchestration-policy.md`, root or context-specific `CONTEXT.md`, optional `TRANSLATION.md`, and ADR routing in code repos, monorepos, and orchestrator workspaces.
...
- Keep `project-memory` issue-tracker setup limited to GitHub, local markdown, orchestrator-local, GitHub-coordination, or custom only; do not add additional hosted-tracker-specific templates to the project-memory flow.
...
- Keep orchestration runtime policy out of `issue-tracker.md`; when needed, store auto-dispatch bounds, worker-surface caps, authorization ceilings, monitoring defaults, and stop-for-owner rules in `project-memory/agents/orchestration-policy.md`.
- Keep behavior-affecting project-memory setup files human-first but table-first: use typed configuration tables for keys that control writes, dispatch, worker surfaces, publication, issue mutation, or closeout, and leave prose for rationale and conventions.
...
- PRDs must include a `Delivery Mode` section using canonical lower-kebab values: `one-feature-branch` for single-repo and monorepo features, `one-pr-per-repo` for true multi-repo work, `one-pr-per-issue` only for isolated exceptions, and `direct-commit` only with explicit authorization. (Codex learning)
- In GitHub and GitHub-coordination tracker modes, hosted issues are authoritative; do not persist repo-local `.scratch/` or `project-memory/features/` mirrors just to feed `gh --body-file`. Use temporary files outside the repo and remove them unless a local mirror or dry-run target is explicitly authorized. (Codex learning)
...
- Generated implementation issues inherit full delivery mode details from their `Source PRD`; issue bodies should still copy the effective `Delivery mode` label as feature-level inherited metadata, then record parallelization, dependencies, closeout, and only issue-level delivery or integration exceptions so orchestrator workers can run slices without inventing branch or PR strategy. (Codex learning)
