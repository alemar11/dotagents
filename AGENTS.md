...
- PRDs must include a `Delivery Topology` section using human-readable labels: `One Feature Branch` for single-repo and monorepo features, `One PR Per Repo` for true multi-repo work, `One PR Per Issue` only for isolated exceptions, and `Direct Commit` only with explicit authorization. (Codex learning)
- Generated implementation issues inherit full delivery topology from their `Source PRD`; issue bodies should record parallelization, dependencies, closeout, and only issue-level topology or integration overrides so orchestrator workers can run slices without inventing branch or PR strategy. (Codex learning)
