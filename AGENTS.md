...
the feature skills that read or write them. Project Memory does not own that
metadata.
...
- Current Codex-dependent skills are `autoreview`, `codex-changelog`, `code-wiki`, `focus-task`, `learn`, `maintainer`, `implement-feature`, and `skill-audit`.
...
- Treat `project-memory` as Codex-aware but portable because optional session-history bootstrap and Code Review Rules history mining are isolated in `skills/project-memory/references/`, while its core setup, review-rule, and internal domain-modeling flows fall back to repository evidence plus optional localization evidence.
...
- Treat `plan-feature` as Codex-dependent and skill-composed because its GitHub issue reads and writes require `$gitstack:github-issues`, while project context uses `$project-memory`, clarification uses `$grill-me-with-context`, and issue hardening uses `$plan-harder`.
...
- Use `project-memory/` as the visible root for durable project memory owned by these runtime skills: `project-memory/adr/` for durable decision records. Use root `CONTEXT.md` as the single context entry point and route optional scoped `CONTEXT.md` files from its `Scoped Contexts` table.

### Project Memory Skill

- Keep `project-memory` as the normal public lifecycle surface for creating or refreshing `AGENTS.md` pointers and the exact `## Code Review Rules` section, plus root and optional scoped `CONTEXT.md`, optional `TRANSLATION.md`, and ADR routing or content in Git repositories.
...
- Keep `skills/project-memory/references/options.md` limited to the genuine controls `memory_slice`, `domain_operation`, and `capture_mode`. Derive execution context from repository evidence, treat a knowledge handoff as input data, and report capture outcome as result state rather than selectable configuration. (Codex learning)
- Always use `AGENTS.md` for short project-memory pointers when an agent-instruction file is needed; keep domain context, planning history, and accepted decisions in `CONTEXT.md` or ADRs.
...
- Keep behavior-affecting contract tables human-first but table-first. Leave prose for rationale and conventions; keep repository identity as runtime evidence rather than Project Memory configuration.
...
- An explicit Project Memory setup or update request authorizes writes to the selected context surfaces; inspection and review requests may return findings without changing files.
...
- Do not restore feature label or issue-type mappings to Project Memory. A future plugin migration may move this contract into its bundled skill tree without changing the consuming vocabulary.
