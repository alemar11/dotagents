...
- The root coordinates App scheduling, its one Goal, repository claims, typed App-operation reconciliation, coarse status, and read-only final verification; one App worker per Feature Spec owns implementation through PR-ready evidence end to end. Root never edits or judges tracker criteria. (Codex learning)
...
- Keep state limited to runs, assignments, canonical repository claims, and typed App-operation reconciliation facts. Do not persist raw Spec/issue bodies, checklists, issue phases, validation attempts, worker technical/domain/provider state, generic request/result payloads, or text hashes. Normal Git head SHAs remain valid evidence. (Codex learning)
- Claim canonical repositories once per root run, atomically for the complete requested set. Use `github:owner/repository` across workspaces and a resolved Git common-directory filesystem identity for local-only repositories. A post-bootstrap blocked owner retains claims; release only at whole-run PR-ready or verified preimplementation abort. (Codex learning)
