...
- Treat `implement-feature` as Codex-dependent and runtime-dependent on `python3` plus local `git` because it is App-only: implementation requires the shipped atomic claim and ledger-cache helpers, explicitly authorized visible ChatGPT desktop app tasks, task-owned Goals, App-managed worktrees, execution-ready Feature Spec bundles, ledgers under `~/.cache/dotagents/skills/implement-feature/ledgers/`, `$autoreview`, and GitStack workflows. It has no planning, root/background implementation, or raw Git worktree machinery.
...
- Keep behavior-affecting setup files human-first but table-first: use typed configuration tables for tracker backend, issue types, and workflow states, and leave prose for rationale and conventions.
...
- Keep `skills/plan-feature/references/options.md` as the owner of the two default-path controls: `mode` and `write_mode`. Consume `tracker_backend` and `repository_layout` as validated Project Memory facts rather than duplicating them as run options. (Codex learning)
...
- `write_mode=apply` publishes through the configured tracker; `write_mode=propose` performs no writes and returns proposed bodies, paths, metadata, and publication order. Never publish incomplete artifacts or mark them agent-ready. (Codex learning)
...
- In `issues-from-existing-spec`, validate and return the durable source body and ref unchanged. Any missing section, schema repair, persisted knowledge payload, or content correction requires a separately authorized Feature Spec update; issue generation must not rewrite or republish the source. Accept `knowledge_delta` only as explicit accepted invocation data kept outside the source. (Codex learning)
- Keep the `plan-feature` issue phase focused on splitting Feature Specs into vertical implementation issues; after graph and scope stabilization it must run `$plan-harder` one or more times per issue, rerun it after material repairs, and persist only the final stable brief plus one provenance line without duplicating top-level sections.
...
- In GitHub tracker mode, hosted issues are authoritative and body files are transient outside the repository. In local mode, durable artifacts live under `planning/features/<feature>/`; proposal output is returned without creating a mirror or durable file.
...
- Keep generated issue type/state in the owning tracker surface: GitHub uses Issue Type and labels, local applied issues use the two canonical header lines, and proposal bodies carry neither while reporting the intended mapping separately. (Codex learning)
...
- Keep every applied multi-repository source and dependency ref globally unambiguous: use `owner/repository#<number>` or a canonical hosted URL for GitHub and `<repository-slug>/<repo-relative-path>` for local Markdown. Propagate the same identity through sibling maps, Feature Dependencies, and issue `source_spec_ref`; proposed refs never become executable. (Codex learning)
...
- Treat one active App orchestrator root as the owner for a project or portfolio source graph. All App runs share the same active-root claim namespace; overlapping roots must stop or perform an explicit takeover/handoff.
- Keep `skills/implement-feature/scripts/active-root-claim` as the sole atomic active-root primitive. Its fixed per-user claim root, filesystem lock, claim JSON, and checksum-bound terminal or durable-handoff release receipts are authoritative; the Markdown ledger is a projection. Do not add adapter fields or claim-root overrides. (Codex learning)
- Keep `skills/implement-feature/scripts/ledger-cache` as the sole ledger archive and retention helper. The root runs it after CLAIM without a visible task or internal subagent, it shares the claim-store lock without mutating claim JSON, and archived ledgers are cold evidence excluded from recovery. (Codex learning)
- Keep runtime orchestration, worker, gate, active-root, target-repo `AGENTS.md`, and ledger details in `skills/implement-feature/SKILL.md` and its references; keep this file limited to dependency and ownership boundaries.
- Begin by proving visible App task creation and App-managed worktree binding, then resolve the mandatory run-scoped task permission. The one permission question discloses and authorizes edit, validation, commit, push, PR publication, current-head review/fixes, CI, tracker closeout preparation, the ready-for-review transition, and automatic deletion of valid archived ledgers older than 180 days after CLAIM. Denial, no answer, or inability to ask aborts before intake or runtime artifacts. (Codex learning)
...
- Keep `pull-request-ready-for-merge-but-not-merged` as `$implement-feature`'s only successful implementation conclusion. It is a fixed invariant, not a user knob: draft PR, uncommitted, commit-only, and push-only states are intermediate or unsupported. After spawn permission and source registration, require GitHub publication/review/CI capability for every affected repository; failure aborts as blocked without target downgrade. Multi-repo intake requires exactly one downstream repo-owned integration Feature Spec with a concrete path-changing integration issue, and completion requires one ready-to-merge PR per affected repository. Merge, post-merge verification, and final tracker closure are outside this skill and require a separate GitHub workflow. (Codex learning)
- Create exactly one App-managed visible task per Feature Spec. Use the fixed action set required to reach the terminal conclusion, narrow it only with affected repositories and allowed paths, and keep implementation, review, fixes, and CI inside that task. The root and every visible task must use the App's `create_goal`, `get_goal`, and `update_goal` tools for their portfolio or assignment Goal; missing Goal tooling is unsupported and must never degrade to an objective-ledger fallback. Root/background implementation and raw Git worktrees remain forbidden. (Codex learning)
...
- Do not accept or execute merge requests inside `implement-feature`. At the fixed conclusion, report the exact PR revisions, checks, armed closing keywords, and external next action, release the claim, and stop. A later merge request starts a separate GitHub workflow rather than resuming orchestration. (Codex learning)
- Require exact-revision Codex review, `$autoreview`, actionable-feedback resolution, and current CI with no review-skip route. Use one 30-minute total review wait deadline per revision and persist one monitoring handoff when the final check remains pending. (Codex learning)
...
- Derive PR count, each terminal PR's default-branch base, and tracker closeout from affected repositories and tracker kind. GitHub closing keywords are armed only on default-branch-base PRs. Local issues move to their exact scoped `done/` paths on the delivery branch after substantive proof; then commit and push the move, run final validation and `$autoreview`, convert drafts to ready-for-review, and obtain current-revision review and CI before terminal merge-ready. The move remains prepared closeout until merge. Direct issue mutation outside that closeout is forbidden.
- Persist active portfolio ledgers under `~/.cache/dotagents/skills/implement-feature/ledgers/`, with one ledger per named portfolio by default. Archive terminal ledgers only after claim release, keep resumable-handoff ledgers active, and automatically prune valid archives older than 180 days with no keep-last floor.
...
- A stale-claim takeover asks permission before task interruption, replaces only roots whose complete repository/source scope the candidate covers, and adopts the original visible tasks rather than dispatching replacements. The helper accepts only schema-5 claims; unsupported claim or takeover state blocks without migration, retirement, or deletion.
...
- Keep ledgers and recovery packets to authorization evidence, source fingerprints, claim ownership, task state, PR/review/CI proof, blockers, and external merge handoff. Remove fixed policy and duplicated bundle fields from runtime state. (Codex learning)
