...
GitHub Actions release workflow owned by `$versioning`. The SemVer and tag
...
- Require an existing stable baseline. Establish the initial version through
 the normal `$versioning` preview and confirmation flow; do not infer it from
...
resolver version is `0.2.2`; it is not this skill's package version or a
