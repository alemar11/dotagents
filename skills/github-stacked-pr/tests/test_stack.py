...
from stack_lib.common import GError, Result

SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "stack"
...
class StackContractTests(unittest.TestCase):
    ...
    def test_cli_json_failure_has_a_structured_secret_safe_envelope(self) -> None:
        ...
        with mock.patch.object(stack, "extension_status", return_value={"status": "missing"}), mock.patch.object(
            ...
            code = __import__("stack_lib.cli", fromlist=["main"]).main(
                ["--json", "ensure", "--install"]
            )
    ...
    def test_shipped_artifact_uses_fake_gh_without_installing_or_contacting_github(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            installed = subprocess.run(
                [str(SCRIPT), "--json", "ensure", "--install"],
                cwd=root,
                env=environment,
                capture_output=True,
                text=True,
            ...
            initialized = subprocess.run(
                [str(SCRIPT), "--json", "init", "--base", "main", "layer-a", "layer-b"],
                cwd=root,
                env=environment,
                capture_output=True,
                text=True,
            ...
            rebased = subprocess.run(
                [str(SCRIPT), "--json", "rebase", "--upstack"],
                cwd=root,
                env=environment,
                capture_output=True,
                text=True,
