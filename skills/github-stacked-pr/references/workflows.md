...
```bash
<skill-root>/scripts/stack --json <command> [args...]
```
...
```bash
<skill-root>/scripts/stack --json init --base main api-layer ui-layer
<skill-root>/scripts/stack --json view --json
```
...
```bash
<skill-root>/scripts/stack --json checkout <stack-number-or-pr-or-url>
```
...
```bash
<skill-root>/scripts/stack --json link <bottom-pr> <top-pr>
```
...
```bash
<skill-root>/scripts/stack --json submit --auto
```
...
Do not use this path for a normal `$yeet` request. It does not create
...
```bash
<skill-root>/scripts/stack --json sync
```
...
```bash
<skill-root>/scripts/stack --json rebase
<skill-root>/scripts/stack --json rebase --upstack
```
...
```bash
<skill-root>/scripts/stack --json rebase --continue
```

If resolution is not safe or the user withdraws the operation:

```bash
<skill-root>/scripts/stack --json rebase --abort
```
...
```bash
<skill-root>/scripts/stack --json merge <pr-or-stack-number> --yes
```
