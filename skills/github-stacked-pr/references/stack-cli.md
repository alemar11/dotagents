...
The skill helper at `<skill-root>/scripts/stack` exposes a thin
...
the skill's host and authentication checks plus `stack ensure`. Load it before
stack-dependent operations; this reference owns the typed wrapper surface and
the explicit installation boundary.

```bash
<skill-root>/scripts/stack --json ensure
<skill-root>/scripts/stack --json ensure --install
```
...
```bash
<skill-root>/scripts/stack --json raw -- view
```
...
verification state. Wrapper failures use stable typed error codes; upstream
...
read, the same noninteractive raw read without the outer `--json` can expose
the sanitized diagnostic, for example `<skill-root>/scripts/stack --json raw -- view --json`. Do not
