...
def doctor() -> dict[str, object]:
    ...
    checks: dict[str, Any] = {
        ...
        "repository": {
            "inside_worktree": bool(root and root.returncode == 0),
            "root": root.stdout.strip() if root and root.returncode == 0 else None,
        },
    }
    gh_stack = extension_status(gh) if gh else extension_status()
    checks["gh_stack"] = gh_stack
...
def doctor_text(payload: dict[str, object], heading: str | None = None) -> str:
    ...
    lines = [heading or str(__version__)]
    lines.extend(
        [
