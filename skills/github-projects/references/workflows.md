...
Use these patterns only after loading [the skill entrypoint](../SKILL.md),
resolving `<skill-root>`, and completing the Projects host, authentication,
and `gh project` capability checks in that entrypoint. The skill's invocation
fields are the sole owner of canonical operation values.
...
`<provider-query>` as provider-native query syntax, not an invocation option.
Pass the complete query as one shell-quoted argument. Resolve an owner-qualified
Project URL to owner type, login, and number, then confirm its node ID and
canonical URL from the exact Project read.
...
Use high-level commands when every value is an exact identity or a typed fact.
Include JSON output where the command supports it.
