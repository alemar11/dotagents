...
class GitStackInstallationParityTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.root = self.home / ".codex/plugins/cache/alemar11/gitstack/5.0.0"
    ...
    def test_exact_installed_500_passes_deterministically(self) -> None:
        self.assertEqual(self.verify(), self.verify())
        self.assertEqual(self.verify()["cli_version"], "5.0.0")

    def test_current_411_and_fake_500_hash_mismatch_reject(self) -> None:
        manifest = self.root / ".codex-plugin/plugin.json"
        manifest.write_text(manifest.read_text().replace('"5.0.0"', '"4.1.1"', 1))
        with self.assertRaisesRegex(PARITY.ParityError, "required 5.0.0"):
            self.verify()
        ...
        with self.assertRaisesRegex(PARITY.ParityError, "cannot report|required 5.0.0"):
            self.verify()
    ...
    def test_manifest_cli_disagreement_source_substitution_and_missing_provenance_reject(self) -> None:
        ...
        package.write_text(package.read_text().replace('version = "5.0.0"', 'version = "4.1.1"'))
