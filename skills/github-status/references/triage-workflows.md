...
not establish exact-head merge readiness; use `$github-status` when
...
closure to `$github-issues` with `mutation_mode=apply`, the exact
...
Ordinary code or disposition investigation needs no skill. Route PR review-thread replies to
`$github-review-threads` with the exact repository and PR,
