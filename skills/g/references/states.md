...
lifecycle, checks, reviews, rules, queues, releases, issue `blockedBy` and
`blocking` relationships, and stars are external provider state. Git branches,
commits, tags, the index, and
the worktree are external Git state. Preserve those authoritative values as
observed instead of translating them into new G states.
