...
The skill-owned artifact at `<skill-root>/scripts/g` exposes a thin
...
<skill-root>/scripts/g --json stack ensure --install
```
...
```bash
<skill-root>/scripts/g --json stack raw -- view
```
...
Normal execution uses `<skill-root>/scripts/g`. The implementation and
tests live under `<skill-root>/projects/g/`; rebuild the shipped artifact with
`<skill-root>/projects/g/scripts/build-artifact`, then re-run `--help`, `--version`,
