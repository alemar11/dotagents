...
Own the evidence-backed metadata choice. The `github-issues` workflow owns every issue
mutation after the choice is exact. When the caller already supplies the label
names or issue type, skip classification and route directly to that workflow.
...
- Read visible issue fields, milestone, assignees, project membership,
  ...
  change them from this workflow.
...
load [`../../gh-dependency-preflight.md`](../../gh-dependency-preflight.md),
then read [Provider reads](provider-reads.md). Distinguish an empty catalog from
a catalog that could not be read.

This workflow performs no direct write transport. Route each authorized
`add-label` or `set-type` operation to the `github-issues` workflow, then independently
...
5. After both the issue and catalog are available, read
   [Classification](classification.md) and derive one proposal from
   the current evidence. Read [States](../../states.md) before returning
...
2. Read [States](../../states.md), inventory the complete visible label
...
3. Inspect repository-owned issue templates, contribution guidance,
   ...
   boundaries. Use the `github-investigation` workflow when this requires deeper code or
   history analysis.
...
5. Read [Taxonomy proposals](taxonomy-proposals.md) and test for
...
- the `github-issues` workflow is required only for authorized `add-label` and `set-type`
  mutations. Classification and dry-run output remain owned by this workflow.
- the `github-investigation` workflow is required when choosing metadata depends on code,
...
- Use the `github-issues` workflow when exact label names or the exact issue type are
  already known, or for any exact authorized label creation, label removal, or
  issue metadata mutation.
- Use the `github-repository-triage` workflow to discover or summarize a repository-wide
  queue when no taxonomy proposal is requested.
- Use the `github-investigation` workflow when classification needs repository evidence
  beyond the issue and its metadata catalog.

## References

- `classification.md`: evidence ranking, selection, drift, and output
  rules. Read after the issue and assignable catalog are available.
- `taxonomy-proposals.md`: repository and issue-corpus evidence,
  gap tests, proposal shapes, and the no-write boundary. Read only for an
  explicit taxonomy proposal.
- `provider-reads.md`: read-only `gh` operations for the target issue,
  label catalog, and organization type catalog.
- `../../states.md`: canonical transient dispositions and external-state
  boundaries. Read before applying or returning a result.
- `../../options.md`: shared canonical G invocation fields.
