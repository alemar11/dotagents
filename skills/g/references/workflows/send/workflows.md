...
```bash
<skill-root>/scripts/g --json publish preflight --repo <owner/repo>
```
...
`github-stack` workflow after Send's publication readback. Do not make
...
Send: the `git-commit` workflow owns the pre-existing-index guard, explicit staging,
staged-diff verification, and commit authoring. Do not let the delegated
`git-commit` workflow call push; Send retains sole ownership of push after its
...
<skill-root>/scripts/g --json publish open --repo <owner/repo> --title-file <absolute-title-file> --body-file <absolute-body-file> --base <pr-base-branch> --draft --expected-worktree-fingerprint <sha256>
```
...
the `github-review-threads` workflow separately using this exact publication evidence;
...
relationships, or closure to the `github-issues` workflow with
...
the `github-issues` workflow or direct read-only `gh issue view` / `gh issue list`.
...
editing. Include only the authorized `title` and/or complete `body` fields in
the request file. Never silently retarget a PR or change its `isDraft` value. Preserve
...
also needs a stack relationship, invoke the `github-stack` workflow separately after this
...
the `github-actions` workflow or the `github-review-threads` workflow after the
