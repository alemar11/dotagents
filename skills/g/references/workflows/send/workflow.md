...
Publish local work from a checkout. This workflow composes focused G
workflows, direct `git`, the shared CLI, and authenticated `gh` PR operations:

- Use the `git-commit` workflow for staging and commit authoring.
- Use `<skill-root>/scripts/g publish preflight` for structured local
...
- When the publish path is selected, load
  ...
  to the `github-stack` workflow.
- Use the `github-issues` workflow, the `github-repository-triage` workflow, the `github-investigation` workflow, the `github-actions` workflow,
  or the `github-review-threads` workflow only for focused follow-up GitHub work.

Use the `<skill-root>` resolved by the active G entrypoint. The CLI uses direct
`git` and authenticated `gh` for its preflight and provider commands.
...
the full publish flow. Route that work to the `github-issues` workflow, perform the
...
- If a target base is explicitly supplied, publish or update the current PR
  ...
  separate `github-stack` workflow concern.
- If there is no publishable local change, stop early and route issue-only
  follow-up to the `github-issues` workflow.
...
parent/child relationship invokes the `github-stack` workflow separately after Send's
...
closeout. See `workflows.md` for body construction and verification.
...
5. Reuse the current commit when it already represents the intended scope.
   Otherwise create one through the `git-commit` workflow with
...
8. Stop after publication and its read-backs. Send must not request or wait for
   ...
   the `github-review-threads` workflow as a separate operation using the exact repository,
...
- `workflows.md`: publish, existing-PR, and retry workflows.
