...
Use the `github-repository-triage` workflow for current-repo queue summaries, the `github-issues` workflow for
authorized issue lifecycle changes, the `github-actions` workflow for Actions failures, and
the `github-review-threads` workflow for listing, drafting, or posting replies to PR review
threads.

## Start

Use the `<skill-root>` resolved by the active G entrypoint before invoking the shared CLI.

Prefer exact `gh` PR or issue context plus local repository evidence over web
browsing. Use direct `git` for local branch discovery:
...
Route review work to the `github-review-threads` workflow with the exact repository
...
changes, or closure through the `github-issues` workflow after normalizing each
...
- provider/channel/integration owner code -> generic core only when multiple owners
...
refactor_disposition: <canonical value from ../../states.md>
refactor_shape: <specific shape|not-applicable>
Proof: <tests/live/CI/source/dependency docs>
Risk: <remaining uncertainty>
```

Use the canonical invocation fields from `../../options.md` for
routed G operations. `refactor_disposition` is a judgment returned by
this workflow, not an invocation option: derive it from the review evidence and do
not ask the user to select it. Keep the explanation of why a refactor is or is
not warranted in the surrounding prose.

## References

- `../../states.md`: provenance confidence and refactor disposition.
- `../../options.md`: caller-selectable G invocation fields.
