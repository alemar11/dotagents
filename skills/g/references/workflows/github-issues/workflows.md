...
policy before invoking this workflow. `mutation_mode=apply` plus the exact target
...
relationship afterward through the native exact-target operations below when
requested, then verify the complete issue state.
...
Use the `<skill-root>` resolved by the active G entrypoint. Preview the
validated local file and Markdown placement
...
and owns whether a non-success result blocks its wider operation. This workflow
