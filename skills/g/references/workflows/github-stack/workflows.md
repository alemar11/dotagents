...
```bash
<skill-root>/scripts/g --json stack <command> [args...]
```

Run the network preflight and extension readiness checks from `<skill-root>/SKILL.md` first.
...
```bash
<skill-root>/scripts/g --json stack init --base main api-layer ui-layer
<skill-root>/scripts/g --json stack view --json
```
...
```bash
<skill-root>/scripts/g --json stack checkout <stack-number-or-pr-or-url>
```
...
```bash
<skill-root>/scripts/g --json stack link <bottom-pr> <top-pr>
```
...
this workflow owns the explicit parent/current relationship. Do not use `link` to
...
```bash
<skill-root>/scripts/g --json stack submit --auto
```
...
Do not use this path for a normal `send` workflow request. It does not create
...
```bash
<skill-root>/scripts/g --json stack sync
```
...
```bash
<skill-root>/scripts/g --json stack rebase
<skill-root>/scripts/g --json stack rebase --upstack
```
...
```bash
<skill-root>/scripts/g --json stack rebase --continue
```

If resolution is not safe or the user withdraws the operation:

```bash
<skill-root>/scripts/g --json stack rebase --abort
```
...
```bash
<skill-root>/scripts/g --json stack merge <pr-or-stack-number> --yes
```
