...
Use this workflow for stack-level work through `<skill-root>/scripts/g
...
Use the `<skill-root>` resolved by the active G entrypoint. Read
[`../../stack-cli.md`](../../stack-cli.md) before using
...
[`workflows.md`](workflows.md) for the requested lifecycle
operation.

## Boundaries

- Use the `send` workflow for publishing or updating one PR, including its title,
  body, closing issue references, draft state, and push ownership.
- Use this workflow for an explicit parent/child link, even when the PRs were just
  published by the `send` workflow. `send` does not infer or invoke `stack link`; do not
  replace this explicit relationship flow with `stack submit`.
- Use this workflow when the user explicitly asks for stack-wide publication,
...
| Request | Owner |
| --- | --- |
| One branch/PR publication or update | the `send` workflow |
| Link one new child PR to one existing target PR | the `github-stack` workflow |
| Inspect or navigate an existing stack | the `github-stack` workflow |
| Publish all active stack branches | the `github-stack` workflow with explicit `stack submit --auto` |
| Rebase, sync, push, restructure, unstack, or merge a stack | the `github-stack` workflow |

For detailed procedures and failure recovery, read
[`workflows.md`](workflows.md). For the typed wrapper
contract, read [`../../stack-cli.md`](../../stack-cli.md).
