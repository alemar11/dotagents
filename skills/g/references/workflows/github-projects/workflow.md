...
[Network execution](../../network-execution.md). Before the first provider
operation, load [the GitHub CLI preflight](../../gh-dependency-preflight.md)
and complete its Projects capability and scope checks.
...
1. Resolve one canonical `project_operation` and `mutation_mode` from
   [the invocation registry](../../options.md). Direct create, edit, add,
   archive, link, close, reopen, copy, mark, unmark, clear, or delete language
   authorizes only the named operation and target. Otherwise remain in
   `dry-run`.
...
6. Read the exact target back after every attempted mutation. Assign one
   `project_operation_result` from [the state registry](../../states.md).
   After an ambiguous response, reconcile once; never replay the mutation.
...
Read [workflows.md](workflows.md) for direct `gh` command patterns, safe
