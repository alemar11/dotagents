...
An authoritative empty check rollup is successful `no-checks` inspection
...
Use the `<skill-root>` resolved by the active G entrypoint. Use the shared CLI when stable aggregation or focused snippets add
value:
...
<skill-root>/scripts/g --json ci inspect --repo <owner/repo> --pr <n>
...
explicitly asked G to write. See
...
- `workflows.md`: GitHub CLI CI workflows.
- `configuration.md`: repository Actions settings, workflow
  permissions, and the read-only permissions preflight.
- `../../states.md`: canonical `g ci inspect` result summaries.
- `script-summary.md`: shared `g ci inspect` contract.
