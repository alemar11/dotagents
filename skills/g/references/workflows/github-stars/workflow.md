...
Use `<skill-root>/scripts/g stars`, backed by authenticated `gh`, for every
provider read and write in this workflow.
...
Manage authenticated-user stars and star lists with `<skill-root>/scripts/g stars`. This workflow
...
- `workflows.md`: star and star-list workflows.
- `script-summary.md`: `stars` command contract.
