...
the `send` workflow instead. `commit_operation=push-only` never creates a commit;
...
follow-up issue creation, or manual closure to the `github-issues` workflow.

## Trigger Cues

Use this workflow for short or implicit commit-authoring asks such as:
...
If the request expands into branch publication or PR creation, route to the `send` workflow
instead of stretching this workflow.
...
and inspect the target as described in `workflows.md`. Do not pass
...
2. If unrelated changes are already staged, stop by default without resetting
   ...
   with the path-limited workflow in `workflows.md` when that scope
...
8. Run the command for the selected kind from `workflows.md`.
...
- `workflows.md`: commit, split-commit, and push-only workflows.
