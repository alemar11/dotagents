...
Use this workflow when a project needs a release version, a Git tag, a release
...
This workflow owns the convention, read-only calculation, and explicit migration
...
the `github-releases` workflow; those requests do not authorize a new version or tag.
...
noncanonical tag to the `github-releases` workflow, even when the user insists, confirms
...
7. After confirmation and successful revalidation, prefer a compatible
   ...
   `controller-dispatch.md` before dispatching it. When no
   compatible controller exists, use the direct tag workflow owned by
   the `github-releases` workflow. Then verify the resulting ref and commit. A final tag
...
this workflow's release-mutation path; inspect them or use the `github-releases` workflow
...
release lifecycle to the `github-releases` workflow:
...
2. If the user asks to apply a tag and the repository already has a compatible
   ...
   `controller-dispatch.md`, select the semantic operation from
...
3. If the release flow will be implemented in GitHub Actions, run the
   ...
   `../github-actions/configuration.md`, then read
   `github-actions.md` before creating or upgrading the workflow.
...
portable topology from `github-actions.md` and copy
...
manual-recovery contracts in `github-actions.md`.
...
- [SemVer and tag format](semver.md)
- [Suggestion states](../../states.md)
- [Existing release-controller dispatch](controller-dispatch.md)
- [GitHub Release lifecycle and notes](../github-releases/workflows.md)
- [GitHub Actions release workflow authoring](github-actions.md)
- [GitHub Actions configuration preflight](../github-actions/configuration.md)
