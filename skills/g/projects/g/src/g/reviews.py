...
def _reservation_cache_root() -> Path:
    return _trusted_user_home() / ".cache/dotagents/skills/g/review-mutations"


def _operation_journal_root() -> Path:
    return _trusted_user_home() / ".cache/dotagents/skills/g/review-operations"
...
def _proof(item: dict[str, Any], body: ProviderText, *, target: dict[str, Any], status: str) -> dict[str, Any]:
    ...
    if not isinstance(object_id, int) or not isinstance(url, str) or not url:
        raise GError("GitHub response omitted provider object identity.", code="provider_identity_missing", exit_code=65)
...
def execute_owned_operation(request_file: str, result_file: str, *, mode: str) -> dict[str, Any]:
    ...
    try:
        result = build_operation_result(request=request, start_receipt=start, status=status, outcome=outcome, facts=normalized_facts, evidence_ref=f"g-operation://{request['operation_id']}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    operation_prepare = operation_sub.add_parser("prepare", help="Prepare one immutable G-owned request without side effects.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "doctor":
        ...
        if args.json:
            ...
            print(doctor_text(payload, f"g reviews {VERSION}"))
