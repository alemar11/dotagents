...
from .common import GError, REPO_PATTERN, checked, normalize_remote, run
...
def _find_open_pr(repo: str, branch: str, root: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise GError(f"Could not parse open PR lookup: {exc}") from exc
    if not isinstance(matches, list):
        raise GError("Open PR lookup returned an unexpected response.")
    if not matches:
        return None
    if len(matches) > 1:
        raise GError(
...
def preflight(repo: str | None = None) -> dict[str, Any]:
    ...
    if not origin_repo:
        raise GError("Could not resolve owner/repo from origin.", code="repo_context_missing", exit_code=3)
    if repo and not REPO_PATTERN.fullmatch(repo.strip()):
        raise GError(f"Invalid repository '{repo}'. Use owner/repo.", code="invalid_arguments", exit_code=64)
    ...
    if not branch:
        raise GError("Refusing to publish from a detached HEAD.", code="unsafe_branch", exit_code=65)
    ...
    if not default_branch:
        raise GError("Could not resolve the repository default branch.", code="repo_context_missing", exit_code=3)
    if branch == default_branch:
        raise GError("Refusing to publish from the default branch.", code="unsafe_branch", exit_code=65)
    ...
    if remote_name is not None and merge_ref is not None:
        ...
        if len(counts) != 2 or not all(item.isdigit() for item in counts):
            raise GError("Could not determine upstream ahead/behind state.", code="upstream_mismatch", exit_code=65)
...
def _read_pull_request(repo: str, number: int, root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise GError("Could not parse pull-request read-back.", code="provider_response_invalid", exit_code=65) from exc
    if not isinstance(payload, dict):
        raise GError("Pull-request read-back returned an unexpected response.", code="provider_response_invalid", exit_code=65)
    return payload
...
def _viewer_login(root: Path) -> str:
    ...
    try:
        ...
        raise GError("Could not parse authenticated GitHub identity.", code="provider_identity_missing", exit_code=65) from exc
    login = payload.get("login") if isinstance(payload, dict) else None
    if not isinstance(login, str) or not login:
        raise GError("Could not resolve authenticated GitHub identity.", code="provider_identity_missing", exit_code=65)
    return login
...
def open_pr(
    *,
    ...
    if state["on_default_branch"]:
        raise GError("Refusing to publish from the default branch.", code="unsafe_branch", exit_code=65)
    if state["dirty"]:
        raise GError("Working tree is dirty; commit or stash changes before publishing.", code="dirty_worktree", exit_code=65)
