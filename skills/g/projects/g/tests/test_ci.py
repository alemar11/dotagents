...
class CiInspectContractTests(unittest.TestCase):
    ...
    def test_inspect_pr_failures_reports_no_checks_as_success(self) -> None:
        ...
        self.assertEqual(payload["summary"], "no-checks")
    ...
    def test_inspect_pr_failures_reports_no_failing_checks(self) -> None:
        checks = [{"name": "build", "state": "SUCCESS"}]
        with (
            mock.patch.object(cli, "ensure_gh_available"),
            mock.patch.object(cli, "resolve_pr", return_value="21"),
            mock.patch.object(cli, "fetch_checks", return_value=checks),
        ):
            payload, exit_code = cli.inspect_pr_failures(
                repo="owner/repo",
                repo_root=None,
        ...
        self.assertEqual(exit_code, 0)
        self.assertEqual(payload["summary"], "no-failing-checks")
        self.assertEqual(payload["checkCount"], 1)
        self.assertEqual(payload["failingCount"], 0)

    def test_inspect_pr_failures_reports_failing_checks(self) -> None:
        checks = [{"name": "build", "state": "FAILURE"}]
        analyzed = {"name": "build", "state": "FAILURE", "log": "failed"}
        with (
            mock.patch.object(cli, "ensure_gh_available"),
            mock.patch.object(cli, "resolve_pr", return_value="21"),
            mock.patch.object(cli, "fetch_checks", return_value=checks),
            mock.patch.object(cli, "analyze_check", return_value=analyzed),
        ):
            payload, exit_code = cli.inspect_pr_failures(
                repo="owner/repo",
                repo_root=None,
        ...
        self.assertEqual(payload["summary"], "failing-checks")
    ...
    def test_shipped_artifact_reports_empty_rollup_as_no_checks(self) -> None:
        ...
        self.assertEqual(payload["data"]["summary"], "no-checks")
