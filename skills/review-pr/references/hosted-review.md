...
A draft returns deferred to the caller; this skill never marks it ready. Use
`$github-review-threads` to post `@codex review` bound to the exact commit,
...
Wait through `$github-review-threads` until terminal review evidence, caller stop,
...
Return completed only when `$github-review-threads` establishes a terminal
...
`$github-review-threads` owns recognition of supported clean reactions and other
