...
class FullFlowDryRunFixtureTests(unittest.TestCase):
    ...
    def test_domain_and_triage_handoffs_use_canonical_option_fields(self) -> None:
        ...
        self.assertIn("### Mixed-Field Compatibility Fixture", triage_options)
        self.assertIn("workflow_state: ready-for-agent\nStatus: needs-triage", triage_options)
        self.assertIn("The embedded `State:` is outside the metadata region", triage_options)
        self.assertIn("two different `workflow_state:` values, stop", triage_options)
        self.assertIn("conflicting `Status:` and `State:`", triage_options)
