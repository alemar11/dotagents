...
class FullFlowDryRunFixtureTests(unittest.TestCase):
    ...
    def test_plan_feature_defers_domain_capture_to_final_integration_task(self) -> None:
        ...
        self.assertIn("preserve a required delta in the PRD handoff", plan_feature)
    ...
    def test_project_memory_triage_and_learn_keep_narrow_authority(self) -> None:
        ...
        self.assertIn("single public invocation for durable memory changes", project_memory)
        self.assertIn("`implementation-closeout`", project_memory)
        self.assertIn("`operation: inline-update`", project_memory)
        self.assertIn("direct `$grill-me-with-context`", project_memory)
        self.assertIn("authorized `inline-update`", project_memory)
        ...
        self.assertIn("`operation: inline-update`", triage)
