...
└── Boost: <short title>       gpt-5.6-sol / medium / same project / local
    ...
    ├── ...
    └── Worker N: <short title>  gpt-5.6-luna / max / same project / local
        where 0 <= N <= 5
```
...
1. Reconstruct the current objective from the parent handoff and keep the
   ...
   orchestrator or invoke Boost. This prohibition is absolute even if the
   orchestrator receives a later explicit request to use Boost.
