...
def _reservation_cache_root() -> Path:
    # Historical v1 durable root from the G plugin era. These one-use markers are
    # reservation state, not a rebuildable cache: changing the path without an
    # explicit migration would hide consumed markers and allow replay.
    return _trusted_user_home() / ".cache/dotagents/plugins/g/review-mutations"


def _operation_journal_root() -> Path:
    # Historical v1 durable root from the G plugin era. Operation journals are
    # durable state tied to g-review-operation:* schemas and owner "g"; do not
    # relocate without an explicit migration.
