...
<skill-root>/scripts/reviews --json operation prepare --controller-envelope-file <absolute-controller-json> --input-file <absolute-input-json> --request-output <absolute-request-json>
<skill-root>/scripts/reviews --json operation validate-request --request-file <absolute-request-json>
<skill-root>/scripts/reviews --json operation execute --request-file <absolute-request-json> --result-output <absolute-result-json>
<skill-root>/scripts/reviews --json operation reconcile --request-file <absolute-reconciliation-request-json> --result-output <absolute-result-json>
<skill-root>/scripts/reviews --json operation validate-result --request-file <absolute-request-json> --result-file <absolute-result-json>
...
<skill-root>/scripts/reviews --json reply --repo <owner/repo> --pr <number> --head <full-40-sha> --comment-id <id> --request-key <request-key> --request-fingerprint <request-fingerprint> --body-file <absolute-message-file> --reservation-file <absolute-reservation-file>
<skill-root>/scripts/reviews --json resolve --repo <owner/repo> --pr <number> --head <full-40-sha> --request-key <request-key> --request-fingerprint <request-fingerprint> --reply-receipt-file <absolute-receipt-file> --reservation-file <absolute-reservation-file>
<skill-root>/scripts/reviews --json edit-comment --repo <owner/repo> --pr <number> --kind <conversation-or-review> --comment-id <id> --body-file <absolute-message-file>
<skill-root>/scripts/reviews --json submit-review --repo <owner/repo> --pr <number> --event <approve-or-request-changes-or-comment> --body-file <absolute-message-file>
<skill-root>/scripts/reviews --json comment --repo <owner/repo> --pr <number> --head <full-40-sha> --request-key <request-key> --request-fingerprint <request-fingerprint> --body-file <absolute-message-file> --reservation-file <absolute-reservation-file>
<skill-root>/scripts/reviews --json ready-trigger --provider codex --repo <owner/repo> --pr <number> --head <full-40-sha> --ready-event-id <event-id> --ready-ref <event-url> --ready-at <utc-timestamp> --base-branch <branch> --body-fingerprint <sha256> --output-file <absolute-ready-receipt-file>
<skill-root>/scripts/reviews --json check --provider codex --repo <owner/repo> --pr <number> --head <sha>
...
<skill-root>/scripts/reviews --json terminal-evidence --provider codex --repo <owner/repo> --pr <number> --head <full-40-sha> --request-receipt-file <absolute-receipt-file>
```
...
`reconcile-mutation`, and `reconcile-terminal`. The current review contract
owns the complete `g-review-operation-request:v1` and
`g-review-operation-result:v1` schemas. Those `g-review-*` wire schema ids are
preserved intentionally for compatibility with existing receipts and journals.
`prepare` and both validators are
read-only. `execute` revalidates the exact controller envelope and atomically
writes a `g-review-operation-start:v1` receipt to this skill's journal
...
derives and passes `<caller-owned-duration>`; this skill never replaces, extends,
or segments it.
...
Invoke only through `<skill-root>/scripts/reviews`. Do not run plugin or
maintenance source trees as the runtime helper.
