...
from reviews_lib.review_operation import (
...
from reviews_lib.review_request import build_request as build_provider_request, receipt
from reviews_lib.ready_review import build_ready_trigger
from reviews_lib import reviews
...
class ReviewOperationContractTests(unittest.TestCase):
    ...
    def controller(self, operation: str) -> dict[str, object]:
        return {
            ...
            "packet_template": {
                ...
                "operation": {"owner": "g", "name": operation, "contract_version": "1.0.0"},
                "authority_binding": self.authority(), "required_inputs": [],
                "accepted_result": {"schema": "g-review-operation-result:v1", "operation": operation},
            },
    ...
    def start(self, request_value: dict[str, object]) -> dict[str, object]:
        value = {
            "schema": "g-review-operation-start:v1", "owner": "g",
    ...
    def test_result_status_outcome_and_facts_are_closed(self) -> None:
        ...
        result = build_result(request=request_value, start_receipt=self.start(request_value), status="completed", outcome="clean", facts=facts, evidence_ref="g-operation://clean")
        self.assertEqual(validate_result(result), result)
        for mutation in (
    ...
    def test_result_correlation_rejects_valid_cross_request_receipt(self) -> None:
        ...
        unrelated = build_result(
            ...
            outcome="pending-at-deadline", facts=facts_a, evidence_ref="g-operation://pending",
        )
    ...
    def test_result_correlation_rejects_cross_pr_and_head_artifacts(self) -> None:
        ...
        for target in (
            ...
            result = build_result(
                ...
                outcome="created", facts=facts_a, evidence_ref="g-operation://cross-target",
            )
            self.assertEqual(validate_result(result), result)
            with self.assertRaises(OperationError):
    ...
    def test_validation_descriptor_projects_exact_reconciliation_start(self) -> None:
        ...
        self.assertEqual(descriptor["owner"], "g")
    ...
    def test_warning_descriptor_binds_exact_pending_predecessor(self) -> None:
        ...
        pending = build_result(
            ...
            evidence_ref="g-operation://pending",
        )
        ...
        self.assertEqual(validation_descriptor(warning)["prior_result_effect"], {
            "relationship": "predecessor", "owner": "g",
        ...
        swapped = build_result(
            ...
            evidence_ref="g-operation://other-pending",
        )
        with self.assertRaises(OperationError):
            build_request(
                operation="warning", controller_envelope=self.controller("warning"),
    ...
    def test_two_finding_followup_obligations_reject_cross_swap(self) -> None:
        ...
        findings = build_result(
            request=wait_request, start_receipt=self.start(wait_request),
            status="completed", outcome="findings", facts=facts,
            evidence_ref="g-operation://findings",
        )
    ...
    def test_g_owns_exact_start_journal_and_rejects_second_start(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            ...
            self.assertEqual(started["schema"], "g-review-operation-start:v1")
            self.assertEqual(
