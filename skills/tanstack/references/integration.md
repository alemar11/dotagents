...
Use this reference when a task spans more than one TanStack layer, especially Query + Router, or Query + Router + Start together.

Use this reference for cross-stack composition, not as a substitute for the focused Router or Start references when the task is confined to one domain.

## Reference Selection

Start with the macro-area umbrellas unless the user names a narrower concern:

- `query.md` for cache keys, `queryOptions`, mutations, invalidation, and stale-time policy.
- `router.md` for route trees, file routes, search params, loaders, navigation, auth guards, and code splitting.
...
`charts.md`, `hotkeys.md`, `markdown.md`, `highlight.md`, `virtual.md`,
`store.md`, `db.md`, `ai.md`, `pacer.md`, `devtools.md`, `config.md`, or
`ranger.md`.
...
- If the issue is only Query, route to `query.md`.
- If the issue is only Router, route to `router.md` and its focused
  references.
- If the issue is only Start, route to `start.md` and its focused
  references.
- If the issue is only CLI scaffolding, add-ons, or package metadata discovery,
  route to `cli.md` and its focused references.
- Keep this reference for Query + Router + Start ownership, loader-prefetch alignment, and hydration-boundary decisions.
