...
- title `Idea: <Name>`;
- the concrete label mapped from `artifact_marker: idea` by Project Memory;
...
6. Confirm every configured label required by the accepted set: the Idea-marker
   label for every candidate and the mapped `needs-triage` label when at least
   one candidate was explicitly queued. When a required mapping exists but its
   concrete label does not, the `apply` capture request authorizes creation of
   that exact configured label only. Create and verify every required missing
   label before the first issue; do not create any other taxonomy.

## GitStack Composition

Delegate every mutation to `$gitstack:github-issues`. Translate Capture Idea's
contract at the boundary; do not pass `write_mode` or other caller-owned policy.
For each missing configured Idea-marker or required workflow-state label pass:
...
Create every missing configured label, when necessary, as its own verified
`create-label` operation. A fresh repository with queued Ideas may therefore
need both the configured Idea-marker label and the configured `needs-triage`
label before issue creation. Then create the issue with the final title, safely
