...
- the exact `needs-triage` label from `github-workflow-contract` only when the
...
2. Load `github-workflow-contract` and use its exact Idea-marker and optional
...
6. Confirm every contract label required by the accepted set: the `idea` label
   ...
   `apply` capture request authorizes creation of that exact label only. Create
