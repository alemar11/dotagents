...
def doctor(skill_root: Path) -> dict[str, Any]:
    ...
    sandbox_help_ok, sandbox_help_stdout, sandbox_help_stderr = (
        _command_output(["codex", "sandbox", "--help"])
        if codex_path
        else (False, "", "codex not found")
    )
    required_help = {
        ...
        "restricted_read_permission_profile": (
            "--permission-profile" in sandbox_help_stdout
        ),
    }
    checks["codex"] = {
        "ok": codex_ok and help_ok and sandbox_help_ok and all(required_help.values()),
        "path": codex_path,
        "version": codex_stdout,
        "exec_help": required_help,
        "error": (
            codex_stderr or help_stderr or sandbox_help_stderr
            if not (codex_ok and help_ok and sandbox_help_ok)
            else None
        ),
    }
    try:
