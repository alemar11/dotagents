...
def _node_prompt(
    *,
    ...
    input_paths = {
        artifact: str(_artifact_path(output_root, snapshot, artifact))
...
Clean source snapshot (read-only by contract): {snapshot.snapshot_path}
Read-only run input root: {output_root}
...
def _invoke_agent(
    *,
    ...
    try:
        ...
        _artifact_hashes(staging_root, snapshot, node.output_artifacts)
        ...
        try:
            ...
            pass
        stdout_path = raw_root / f"{node.node_id}-attempt-{attempt}.jsonl"
        stderr_path = raw_root / f"{node.node_id}-attempt-{attempt}.stderr.txt"
        failure = (
            {
                "attempt": attempt,
                "started_at": started_at,
                ...
                "usage": None,
                "duration_ms": 0,
                "exit_code": exc.exit_code if isinstance(exc, ExecutionError) else None,
...
def _prepare(
    ...
    return {
        ...
        "status": "pass",
        "input_artifacts": input_hashes,
        "output_artifacts": _artifact_hashes(output_root, snapshot, node.output_artifacts),
        "source_tree_sha256_before": before,
        "source_tree_sha256_after": after,
        "usage": None,
        "duration_ms": 0,
...
def _mark_agent_result(
    ...
    node_state["status"] = "pass" if result else "fail"
