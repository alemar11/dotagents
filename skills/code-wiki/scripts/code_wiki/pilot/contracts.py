...
EXPECTED_GRAPH_NODES = {
    "baseline": frozenset({"prepare", "baseline-generate", "validate", "reader"}),
    "node-graph": frozenset({"prepare", "study", "render", "validate", "repair", "reader"}),
}
RETIRED_CANDIDATE_NODE_IDS = frozenset(
    {"study-architecture", "study-interfaces", "study-operations", "synthesize"}
)
...
def load_graph(skill_root: Path, mode: str) -> GraphContract:
    ...
    if mode == "node-graph":
        retired = sorted(path.stem for path in paths if path.stem in RETIRED_CANDIDATE_NODE_IDS)
        if retired:
            raise ContractError("retired candidate node contracts are forbidden: " + ", ".join(retired))
    nodes: dict[str, NodeContract] = {}
    for path in paths:
    ...
    expected_nodes = EXPECTED_GRAPH_NODES[mode]
    if set(nodes) != expected_nodes:
        missing = sorted(expected_nodes - set(nodes))
        extra = sorted(set(nodes) - expected_nodes)
        details = []
        if missing:
            details.append("missing=" + ",".join(missing))
        if extra:
            details.append("extra=" + ",".join(extra))
        raise ContractError(f"{mode} graph node set is invalid: {'; '.join(details)}")
