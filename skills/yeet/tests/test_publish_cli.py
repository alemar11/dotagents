...
from publish_lib import __version__
from publish_lib import cli
from publish_lib.common import GError, Result
from publish_lib.publish import _find_open_pr, open_pr, preflight
...
class CliContractTests(unittest.TestCase):
    ...
    def test_json_doctor_shape(self) -> None:
        doctor_payload = {'ok': True, 'provider_ready': True, 'version': __version__, 'checks': {'gh': {'ok': True}}}
    ...
    def test_json_argument_error(self) -> None:
        code, output = self.invoke(['--json', 'snapshot', '--repo', 'bad'])
        payload = json.loads(output)
    ...
    def test_publish_rejects_inline_title_without_echoing_it(self) -> None:
        ...
        code, output = self.invoke(['--json', 'open', '--title', hostile, '--body-file', '/tmp/body.md'])
        self.assertEqual(code, 64)
    ...
    def test_publish_template_command_was_removed(self) -> None:
        code, output = self.invoke(['--json', 'template'])
        self.assertEqual(code, 64)
        self.assertEqual(json.loads(output)['error']['code'], 'invalid_arguments')
    ...
    def test_publish_refuses_default_branch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch('publish_lib.publish.preflight', return_value=state):
    ...
    def test_publish_dry_run_accepts_explicit_non_default_base(self) -> None:
        state = {'repo': 'owner/repo', 'root': '/tmp/repo', 'branch': 'child', 'default_branch': 'main', 'on_default_branch': False, 'upstream': 'origin/child', 'dirty': False, 'status': [], 'existing_pull_request': None}
        with tempfile.TemporaryDirectory() as directory:
            title_file = Path(directory) / 'title.txt'
            body_file = Path(directory) / 'body.md'
            title_file.write_text('Title', encoding='utf-8')
            ...
            with mock.patch('publish_lib.publish.preflight', return_value=state):
                result = open_pr(repo=None, title_file=str(title_file), body_file=str(body_file), draft=True, base='parent', dry_run=True, expected_worktree_fingerprint=None)
        self.assertEqual(result['target']['base'], 'parent')

    def test_reused_pr_preserves_existing_base_and_draft_state(self) -> None:
        state = {'repo': 'owner/repo', 'root': '/tmp/repo', 'branch': 'child', 'default_branch': 'main', 'on_default_branch': False, 'upstream': 'origin/child', 'dirty': False, 'status': [], 'existing_pull_request': {'number': 7, 'baseRefName': 'parent', 'isDraft': False}}
        pull = {'number': 7, 'html_url': 'https://github.com/owner/repo/pull/7', 'title': 'Title', 'body': 'Closes #10', 'draft': False, 'head': {'ref': 'child', 'repo': {'full_name': 'owner/repo'}}, 'base': {'ref': 'parent', 'repo': {'full_name': 'owner/repo'}}}
        with tempfile.TemporaryDirectory() as directory:
            title_file = Path(directory) / 'title.txt'
            body_file = Path(directory) / 'body.md'
            title_file.write_text('Title', encoding='utf-8')
            body_file.write_text('Closes #10', encoding='utf-8')
            with mock.patch('publish_lib.publish.preflight', return_value=state), mock.patch('publish_lib.publish._read_pull_request', return_value=pull):
                result = open_pr(repo=None, title_file=str(title_file), body_file=str(body_file), draft=True, base=None, dry_run=False, expected_worktree_fingerprint=None)
    ...
    def test_reused_pr_rejects_explicit_base_drift(self) -> None:
        state = {'repo': 'owner/repo', 'root': '/tmp/repo', 'branch': 'child', 'default_branch': 'main', 'on_default_branch': False, 'upstream': 'origin/child', 'dirty': False, 'status': [], 'existing_pull_request': {'number': 7, 'baseRefName': 'parent', 'isDraft': True}}
        with tempfile.TemporaryDirectory() as directory:
            title_file = Path(directory) / 'title.txt'
            body_file = Path(directory) / 'body.md'
            title_file.write_text('Title', encoding='utf-8')
            ...
            with mock.patch('publish_lib.publish.preflight', return_value=state):
                with self.assertRaises(GError) as raised:
                    open_pr(repo=None, title_file=str(title_file), body_file=str(body_file), draft=True, base='main', dry_run=False, expected_worktree_fingerprint=None)
    ...
    def test_preflight_keeps_matching_explicit_repo_checkout(self) -> None:
        ...
        def fake_run(command, cwd=None):
            ...
            raise AssertionError(command)
        with mock.patch('publish_lib.publish.checked', side_effect=fake_checked), mock.patch('publish_lib.publish.run', side_effect=fake_run):
    ...
    def test_preflight_rejects_explicit_repo_origin_mismatch(self) -> None:
        with mock.patch('publish_lib.publish.checked', side_effect=[Result(0, '/tmp/repo\n', ''), Result(0, 'git@github.com:owner/other.git\n', '')]):
            with self.assertRaises(GError) as raised:
    ...
    def test_preflight_rejects_wrong_upstream(self) -> None:
        checked_results = [Result(0, '/tmp/repo\n', ''), Result(0, 'git@github.com:owner/repo.git\n', ''), Result(0, '', ''), Result(0, 'feature\n', ''), Result(0, '## feature...fork/feature\n', ''), Result(0, 'main\n', '')]
        run_results = [Result(0, 'fork\n', ''), Result(0, 'refs/heads/feature\n', '')]
        with mock.patch('publish_lib.publish.checked', side_effect=checked_results), mock.patch('publish_lib.publish.run', side_effect=run_results):
            with self.assertRaises(GError) as raised:
                preflight()
    ...
    def test_preflight_allows_missing_upstream_before_first_push(self) -> None:
        checked_results = [Result(0, '/tmp/repo\n', ''), Result(0, 'git@github.com:owner/repo.git\n', ''), Result(0, '', ''), Result(0, 'feature\n', ''), Result(0, '## feature\n', ''), Result(0, 'main\n', ''), Result(0, '[]\n', '')]
        with mock.patch('publish_lib.publish.checked', side_effect=checked_results), mock.patch('publish_lib.publish.run', return_value=Result(1, '', 'no upstream')):
            state = preflight()
    ...
    def test_publish_open_rejects_unpushed_branch(self) -> None:
        state = {'repo': 'owner/repo', 'root': '/tmp/repo', 'branch': 'feature', 'default_branch': 'main', 'on_default_branch': False, 'upstream': None, 'upstream_valid': True, 'needs_push': True, 'dirty': False, 'status': [], 'existing_pull_request': None}
        with tempfile.TemporaryDirectory() as directory:
            title_file = Path(directory) / 'title.txt'
            body_file = Path(directory) / 'body.md'
            title_file.write_text('Title', encoding='utf-8')
    ...
    def test_open_pr_lookup_requires_verified_head_identity(self) -> None:
        payload = json.dumps([{'number': 7, 'url': 'https://github.com/owner/repo/pull/7', 'title': 'PR', 'state': 'OPEN', 'isDraft': True, 'headRefName': 'feature', 'headRepositoryOwner': {'login': 'other'}, 'headRepository': {'name': 'repo'}}])
        with mock.patch('publish_lib.publish.checked', return_value=Result(0, payload, '')):
            with self.assertRaises(GError) as raised:
    ...
    def test_open_pr_lookup_preserves_existing_base_branch(self) -> None:
        payload = json.dumps([{'number': 7, 'url': 'https://github.com/owner/repo/pull/7', 'title': 'PR', 'state': 'OPEN', 'isDraft': True, 'headRefName': 'feature', 'baseRefName': 'main', 'headRepositoryOwner': {'login': 'owner'}, 'headRepository': {'name': 'repo'}}])
        with mock.patch('publish_lib.publish.checked', return_value=Result(0, payload, '')):
    ...
    def test_malformed_publish_response_recovers_from_one_exact_head_read_back(self) -> None:
        state = {'repo': 'owner/repo', 'root': '/tmp/repo', 'branch': 'feature', 'default_branch': 'main', 'on_default_branch': False, 'upstream': 'origin/feature', 'needs_push': False, 'dirty': False, 'status': [], 'existing_pull_request': None}
        pull = {'number': 7, 'html_url': 'https://github.com/owner/repo/pull/7', 'title': '`title` $HOME', 'body': '$(command)\nUnicode ✓', 'draft': True, 'created_at': '2026-07-20T12:00:00Z', 'user': {'login': 'agent'}, 'head': {'ref': 'feature', 'repo': {'full_name': 'owner/repo'}}, 'base': {'ref': 'main', 'repo': {'full_name': 'owner/repo'}}}
        with tempfile.TemporaryDirectory() as directory:
            title_file = Path(directory) / 'title.txt'
            body_file = Path(directory) / 'body.md'
            title_file.write_text(pull['title'], encoding='utf-8')
            body_file.write_text(pull['body'], encoding='utf-8')
            with mock.patch('publish_lib.publish.preflight', return_value=state), mock.patch('publish_lib.publish._viewer_login', return_value='agent'), mock.patch('publish_lib.publish.api_request', return_value=Result(0, 'not-json', '')) as mutation, mock.patch('publish_lib.publish._find_open_pr', return_value={'number': 7}) as lookup, mock.patch('publish_lib.publish._read_pull_request', return_value=pull) as read_back, mock.patch('publish_lib.publish.datetime') as clock:
                clock.now.return_value = datetime(2026, 7, 20, 12, 0, 0, tzinfo=timezone.utc)
                result = open_pr(repo=None, title_file=str(title_file), body_file=str(body_file), draft=True, base='main', dry_run=False, expected_worktree_fingerprint=None)
    ...
    def test_malformed_publish_response_missing_or_mismatched_read_back_is_ambiguous(self) -> None:
        ...
        title = '`title` $HOME'
        body = '$(command)\nUnicode ✓'
        mismatched = {'number': 7, 'html_url': 'https://github.com/owner/repo/pull/7', 'title': title, 'body': 'different', 'draft': True, 'created_at': '2026-07-20T12:00:00Z', 'user': {'login': 'agent'}, 'head': {'ref': 'feature', 'repo': {'full_name': 'owner/repo'}}, 'base': {'ref': 'main', 'repo': {'full_name': 'owner/repo'}}}
        with tempfile.TemporaryDirectory() as directory:
            title_file = Path(directory) / 'title.txt'
            body_file = Path(directory) / 'body.md'
            title_file.write_text(title, encoding='utf-8')
            ...
            for lookup_result, read_result in ((None, mismatched), ({'number': 7}, mismatched)):
                with self.subTest(lookup_result=lookup_result), mock.patch('publish_lib.publish.preflight', return_value=state), mock.patch('publish_lib.publish._viewer_login', return_value='agent'), mock.patch('publish_lib.publish.api_request', return_value=Result(0, 'not-json', '')), mock.patch('publish_lib.publish._find_open_pr', return_value=lookup_result) as lookup, mock.patch('publish_lib.publish._read_pull_request', return_value=read_result) as read_back, mock.patch('publish_lib.publish.datetime') as clock:
                    clock.now.return_value = datetime(2026, 7, 20, 12, 0, 0, tzinfo=timezone.utc)
                    with self.assertRaises(GError) as raised:
                        open_pr(repo=None, title_file=str(title_file), body_file=str(body_file), draft=True, base='main', dry_run=False, expected_worktree_fingerprint=None)
