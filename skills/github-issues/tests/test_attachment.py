...
from attachment_lib import __version__, attachment, cli
from attachment_lib.common import GError
...
class AttachmentContractTests(unittest.TestCase):
    ...
    def test_dry_run_is_local_and_returns_only_file_proof(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch("attachment_lib.attachment.subprocess.run") as run, mock.patch(
                "attachment_lib.attachment.request.urlopen"
            ) as urlopen:
    ...
    def test_upload_resolves_rest_id_and_posts_opaque_binary_body(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch(
                "attachment_lib.attachment.subprocess.run", side_effect=self._provider_results()
            ) as run:
                result = attachment.upload(
                    repo="owner/repo",
                    file=str(path),
    ...
    def test_repository_identity_must_be_numeric_and_exact(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            path = self._file(directory)
            with mock.patch("attachment_lib.attachment.subprocess.run", return_value=malformed):
    ...
    def test_shipped_artifact_supports_local_attachment_dry_run(self) -> None:
        artifact = Path(__file__).resolve().parents[1] / "scripts" / "attachment-upload"
        with tempfile.TemporaryDirectory() as directory:
