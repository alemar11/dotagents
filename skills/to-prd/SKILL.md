...
Treat the PRD as the canonical source for full delivery mode and branch/PR
details. Schedule ordering and wave unlock conditions belong in
`delivery-plan.md`, generated by `to-issues`; generated issues should copy the
effective `Delivery mode` label from this section for scheduler visibility, marked
as feature-level and inherited from `Source PRD`, while leaving full branch/PR
details here unless an issue has an explicit exception or cross-repo closeout rule.
