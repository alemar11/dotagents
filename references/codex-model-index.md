...
| [`$se:study`](../plugins/se/skills/study/SKILL.md) | `gpt-5.6-luna` | `max` | Up to five native subagents under the active Study controller on either surface. Study creates no visible App worker tasks; the cap and transport policy are owned by [`plugins/se/skills/study/SKILL.md`](../plugins/se/skills/study/SKILL.md). |
...
| [`$se:implement`](../plugins/se/skills/implement/SKILL.md) | `gpt-5.6-sol` | `xhigh` | Fresh independent read-only local reviewer execution for each stable committed Feature candidate; the review covers the complete delta and must be clean for the exact base and full HEAD before publication. |
