...
| [`$se2:implement`](../plugins/se2/skills/implement/SKILL.md) | `gpt-5.6-sol` | `medium` | Implement orchestrator role; the two-role profile is owned by [`skills/implement/references/task-profile.md`](../plugins/se2/skills/implement/references/task-profile.md), with no fallback to a worker role. |
| [`$se2:implement`](../plugins/se2/skills/implement/SKILL.md) | `gpt-5.6-luna` | `max` | Implement worker role; the live preflight must verify this role before the orchestrator starts, with no automatic substitution or downgrade. |
