...
| [`$se2:implement`](../plugins/se2/skills/implement/SKILL.md) | `gpt-5.6-sol` | `medium`/`high`/`xhigh` adaptive | One Feature Worker per implementation-eligible Feature Plan member; it derives execution units from the textual plan, implements them in one worktree, and performs exact-HEAD in-session review with the same resolved reasoning. |
