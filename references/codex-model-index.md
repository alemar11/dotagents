...
| [`$se2:implement`](../plugins/se2/skills/implement/SKILL.md) | `gpt-5.6-sol` | `medium`/`high`/`xhigh` adaptive | One implementation worker per eligible Feature Task; the worker also performs exact-HEAD in-session review with the same resolved reasoning. |
