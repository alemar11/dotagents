...
| [`$se:feature`](../plugins/se/skills/feature/SKILL.md) | `gpt-5.6-sol` | `medium` | Required Feature Plan planner and reducer; its complete fixed profile is actively requested and never inherited. The profile is owned by [`skills/feature/references/task-profile.md`](../plugins/se/skills/feature/references/task-profile.md), while the root preflight verifies required task capabilities and records optional delegation and goal facts. |
| [`$se:feature`](../plugins/se/skills/feature/SKILL.md) | `gpt-5.6-sol` | `medium` | Bounded study analysis-worker role for intent, context, boundary, question, and evidence analysis. It is delegated for substantial planning when capability is available; any separately instantiated task receives the complete explicit profile, while unavailable delegation falls back to the planner running the same lens serially. |
| [`$se:feature`](../plugins/se/skills/feature/SKILL.md) | `gpt-5.6-sol` | `medium` | Independent critic-analyst role, delegated for substantial planning when capability is available. Any separately instantiated task receives the complete explicit profile, studies the original problem without the planner draft during its first pass, confirms any substantial question-free brief, then independently reviews the complete draft before Plan Validation and returns read-only findings to the parent reducer. |
