...
| [`$se:deliver`](../plugins/se/skills/deliver/SKILL.md) | `gpt-5.6-luna` | `max` | Local [worker contract](../plugins/se/skills/deliver/references/workers.md) owns App tasks and CLI subagents for contribution, integration and PR delivery assignments; explicit user overrides win. |
...
| [`$se:deliver-features`](../plugins/se/skills/deliver-features/SKILL.md) | `gpt-5.6-luna` | `max` | Shared [`developer`](../plugins/se/references/subagents/developer.md) role: a run-scoped visible task in an isolated saved-project worktree on the App surface, or a native subagent in an isolated worktree on the CLI surface. The coordinator owns selection, publication phases and recovery. |
| [`$se:deliver-features`](../plugins/se/skills/deliver-features/SKILL.md) | `gpt-6-astra` | `medium` | Shared [`code-reviewer`](../plugins/se/references/subagents/code-reviewer.md) native subagent for an immutable candidate; the composing coordinator owns snapshot/receipt validation and the shared per-PR repair budget. |
